...
export const BadgePill: React.FC<BadgePillProps> = ({
  ...
  return (
    ...
    >
      <div
        className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full border text-[10px] font-bold font-sans transition-all duration-200 ${colorClasses} ${earned ? "cursor-pointer hover:scale-105" : "cursor-not-allowed select-none"}`}
      >
      ...
      {showTooltip && (
        <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 w-56 p-2.5 bg-slate-900/98 text-white rounded-xl text-xs shadow-xl backdrop-blur-xs transition duration-150 animate-in fade-in slide-in-from-bottom-2 z-50 pointer-events-none text-left">
          <div className="flex items-center gap-1.5 font-bold mb-1 border-b border-white/10 pb-1">
            <span className={`${earned ? "text-amber-400" : "text-slate-400"}`}>
              {earned ? "🏆 Verified Badge" : "🔒 Locked Badge"}
            </span>
          </div>
          ...
          return (
            ...
            >
              ...
              <div className="space-y-2">
                <div
                  className={`w-8 h-8 rounded-xl flex items-center justify-center border font-bold ${headerColors} ${earned ? "animate-in zoom-in-95" : ""}`}
                >
                ...
                <div className="space-y-1">
                  <h4
                    className={`font-black text-xs leading-snug ${earned ? "text-slate-800" : "text-slate-500"}`}
                  >
                    {(title || "").split(" (")[0]}
                  </h4>
