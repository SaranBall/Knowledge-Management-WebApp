...
interface LearningCenterProps {
  ...
  setKmContributionLogs: React.Dispatch<
    React.SetStateAction<KMContributionLog[]>
  >;
  onAwardPoints: (
    userId: string,
...
export const LearningCenter: React.FC<LearningCenterProps> = ({
  ...
  const [selectedOnboardingCourseId, setSelectedOnboardingCourseId] =
    useState<string>(onboardingCourses[0]?.id || "");
  const [selectedGeneralCourseId, setSelectedGeneralCourseId] =
    useState<string>(generalCourses[0]?.id || "");
  ...
  const myCompetencies = userCompetencies.filter(
    (c) => c.userId === currentUser.id,
  );
  const myCertificates = calculateRemainingDays(
  ...
  const handleGeneratePersonalizedPath = async () => {
    ...
    try {
      ...
      const coursesSub = courses.map((c) => ({
        id: c.id,
        title: c.title,
      ...
      if (data.careerGoalExplanation) {
        ...
        normalizedResult.analysis =
          data.analysis || "พร้อมสำหรับการเรียนรู้สายอาชีพ";
      }
      ...
      normalizedResult.phases = rawSteps.map((ph: any) => ({
        ...
        guidelines: ph.targetSkills
          ? `ทักษะเป้าหมาย: ${ph.targetSkills.join(", ")}`
          : ph.guidelines || "",
  ...
  const currentCourseId =
    activeSubTab === "onboarding"
      ? selectedOnboardingCourseId
      : selectedGeneralCourseId;
  ...
  const currentProgress = userProgressList.find(
    (p) => p.userId === currentUser.id && p.courseId === selectedCourse?.id,
  );
  const relevantResult = examResults.find(
    (e) =>
  ...
  // Active user details
  const myCompletedCoursesCount = userProgressList.filter(
    (p) => p.userId === currentUser.id && p.status === "Completed",
  ).length;
  const myExams = examResults.filter(
    (e) => e.employeeId === currentUser.employeeId,
  );
  const myAverageQuizScore =
    myExams.length > 0
      ? Math.round(
          myExams.reduce((sum, current) => sum + current.score, 0) /
            myExams.length,
        )
      : 0;
  ...
  const handleAddQuestionNext = () => {
    ...
    if (qType === "SingleChoice") {
      ...
      const lines = newCourseState.quizOptionsStr
        .split("\n")
        .map((o) => o.trim())
        .filter(Boolean);
      ...
      if (parsedPairs.length === 0) {
        alert(
          '⚠️ สำหรับประเภทจับคู่ กรุณากรอกคู่คำตอบในช่องตัวเลือก รูปแบบ "คำด้านซ้าย = คำด้านขวา" บรรทัดละ 1 คู่\nเช่น:\nชุด PPE = อุปกรณ์ป้องกันส่วนบุคคล',
        );
        return;
      }
      pairs = parsedPairs;
    ...
    alert(
      "➕ เพิ่มคำถามข้อนี้ลงลิสต์แล้ว! คุณสามารถพิมพ์โจทย์ข้อถัดไปต่อได้ทันที",
    );
  };
  ...
  const handleAddCourseSubmit = (e: React.FormEvent) => {
    ...
    const firstLesson: Lesson = {
      ...
      content:
        newCourseState.lessonContent ||
        "รายละเอียดคู่มือการใช้งานเครื่องจักรและสารสนเทศ...",
      durationMinutes: 30,
  ...
  const handleEditCourseSubmit = (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(
        "ข้อมูลโครงสร้างรูปแบบบทเรียนหรือข้อสอบมีข้อผิดพลาดทาง JSON ไวยากรณ์ กรุณาตรวจสอบวงเล็บเหลี่ยมปีกกาอีกครั้ง",
      );
      return;
    }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10.5px] text-slate-550 mt-0.5">
                ปลดล็อคเข็มประดับทักษะพิเศษ{" "}
                <strong className="text-indigo-700">
                  {activeEarnedBadges.length} / {myBadges.length}
                </strong>{" "}
                แฟ้มผลงาน จากประวัติการเรียน ผลสอบ และผลงาน Kaizen
              </p>
            </div>
          </div>
          ...
          >
            {showBadgesPanel
              ? "▲ ซ่อนเกณฑ์คุณวุฒิ"
              : "▼ ดูรายการตราสัญลักษณ์ทั้งหมด"}
          </button>
        </div>

        {/* Small preview of active earned badges */}
        <div className="flex flex-wrap items-center gap-2 pt-1 border-t border-slate-100">
          <span className="text-[9px] uppercase font-bold text-slate-400 tracking-wider mr-1">
            ตราคุณวุฒิตรงสิทธิ์:
          </span>
      ...
      {activeSubTab === "transcript" ? (
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-6 animate-fade-in text-slate-800">
          ...
          <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            ...
            <div className="p-3 bg-white rounded-lg border border-slate-200 text-left md:text-right text-xs shrink-0 font-mono text-slate-550 space-y-0.5">
              ...
              <p className="text-[#a21217] font-bold">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <h5 className="font-extrabold text-[#15329c] text-xs uppercase tracking-wider">
                📊 ระเบียนประวัติคะแนนสอบและสถานะใบเซอร์ฯ (Certification & Exam
                Results Transcripts)
              </h5>
              <span className="text-[10.5px] text-slate-450 italic">
                อัปเดตแบบเรียลไทม์บันทึกเข้าระบบ ISO ตรวจสอบ
              </span>
            </div>
                        ...
                        >
                          ...
                          <td className="p-3">
                            {courseObj?.type === "Onboarding" ? (
                              <span className="px-1.5 py-0.5 bg-blue-50 text-blue-700 text-[10px] rounded border border-blue-200">
                                Onboarding
                              </span>
                            ) : (
                          ...
                          <td className="p-3 text-center">
                            {exam.pass ? (
                              <button
                                onClick={() =>
                                  handleViewHistoricCertificate(exam.courseId)
                                }
            ...
            <div className="border-t-4 border-dashed border-slate-200 pt-6 mt-6 animate-fade-in text-center space-y-4">
              <div className="flex items-center justify-between border-b pb-2 mb-4 bg-slate-50 p-2.5 rounded-xl">
                <span className="font-bold text-xs text-indigo-700 flex items-center gap-1">
                  ...
                  ใบประกาศกิตติคุณรับรองระบบมาตรฐาน:{" "}
                ...
                <div className="bg-gradient-to-br from-[#ffffff] via-[#f7fbfd] to-[#ffffff] p-8 rounded-2xl border-4 border-double border-teal-500 shadow-lg max-w-xl mx-auto space-y-6 relative overflow-hidden text-slate-800">
                  ...
                  <div className="space-y-1 text-center">
                    ...
                    <p className="text-[9px] text-slate-400 font-mono tracking-wider">
                      ROYAL MEIWA PAX CO., LTD. • QUALITY CONTROL & COMPLIANCE
                      SYSTEM
                    </p>
                  </div>
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-[10px] text-slate-450 mt-1 font-mono">
                      ตำแหน่ง: {currentUser.position} | ฝ่ายวิเคราะห์:{" "}
                      {getDepartmentById(currentUser.departmentId)?.name ||
                        currentUser.departmentId}
                    </p>
                  </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-2.5 min-w-0">
                          <span
                            className={`w-5 h-5 rounded-full flex items-center justify-center font-mono text-[10px] font-bold ${idx === 0 ? "bg-amber-100 text-amber-805 text-xs" : idx === 1 ? "bg-slate-200 text-slate-805" : idx === 2 ? "bg-orange-100 text-orange-850" : "bg-slate-50 text-slate-400"}`}
                          >
                            {idx + 1}
                          </span>
                          <span className="truncate text-slate-805 text-[12px]">
                            {entrant.userName}
                          </span>
                          {isCurrentUser && (
                            <span className="bg-[#15329c] text-white font-bold px-1 rounded text-[8.5px] uppercase">
                              คุณ
                            </span>
                          )}
                        </div>
                        <div className="flex items-center gap-3 shrink-0">
                          <span className="text-slate-505 text-[10.5px]">
                            Lvl {entrant.level}
                          </span>
                          <strong className="text-indigo-850 font-mono text-[12px] text-right">
                            {entrant.points} XP
                          </strong>
                        </div>
                      </div>
              ...
              <div className="space-y-3.5">
                <div className="flex justify-between items-center bg-[#fbfbf9] p-3 rounded-xl border border-slate-200/60">
                  ...
                  <span className="bg-indigo-50 text-indigo-805 font-mono text-[9px] px-2 py-0.5 rounded font-black">
                    ISO COMPLIANT
                  </span>
                </div>

                {kmContributionLogs.filter(
                  ...
                  <div className="overflow-y-auto max-h-[220px] pr-1.5 border border-slate-200 rounded-xl divide-y text-xs bg-white">
                    {kmContributionLogs
                      ...
                      .map((log) => {
                        ...
                        if (log.activityType === "COURSE_PERFECT")
                          badgeBg =
                            "bg-amber-50 text-amber-805 border-amber-200";
                        else if (log.activityType === "COURSE_PASS")
                          badgeBg =
                            "bg-emerald-50 text-emerald-800 border-emerald-250";
                        else if (log.activityType === "KB_CONTRIBUTION")
                          badgeBg = "bg-blue-50 text-blue-800 border-blue-200";
                        else if (log.activityType === "OFFLINE_CHECKIN")
                          badgeBg =
                        ...
                        return (
                          <div
                            key={log.id}
                            className="p-3 hover:bg-slate-5/50 flex flex-col sm:flex-row justify-between sm:items-center gap-2"
                          >
                            ...
                            <div className="flex items-center gap-2 shrink-0">
                              <span
                                className={`text-[8.5px] font-extrabold uppercase px-1.5 py-0.5 rounded border ${badgeBg}`}
                              >
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-205 shadow-xs space-y-6 animate-fade-in text-slate-800">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">
                เปรียบเทียบขีดความสามารถปัจจุบันของคุณกับระดับความรู้รับเป้าในตำแหน่ง{" "}
                <strong className="text-[#15329c] font-black">
                  {currentUser.position}
                </strong>{" "}
                (แผนก{" "}
                {getDepartmentById(currentUser.departmentId)?.name ||
            ...
            <div className="bg-slate-50 border border-slate-200/60 rounded-2xl p-3 flex gap-4 text-xs">
              ...
              <div className="text-center">
                ...
                <strong className="text-base text-[#e51a24] font-mono">
                  {
                    myCompetencies.filter(
                      (c) => c.actualLevel < c.expectedLevel,
                    ).length
                  }
                </strong>
              </div>
          ...
          {myCertificates.some(
            (cert) =>
              cert.status === "Expired" || cert.status === "ExpiringSoon",
          ) && (
                ...
                {myCertificates
                  ...
                  .map((cert) => (
                    <p key={cert.id}>
                      ...
                      ({cert.type}) สถานะ:
                      <span
                        className={`mx-1.5 px-2 py-0.5 rounded-full font-bold text-[8.5px] ${cert.status === "Expired" ? "bg-rose-100 text-rose-700" : "bg-amber-100 text-amber-900"}`}
                      >
                        {cert.status === "Expired"
                          ? "หมดอายุถาวร"
            ...
            {myCompetencies.map((comp) => {
              ...
              const recommendedCourse = courses.find(
                (c) =>
                  c.title
                    .toLowerCase()
                    .includes(comp.skillName.toLowerCase()) ||
                  (c.tags &&
                    c.tags.some(
                      (t) =>
                        t.toLowerCase() === comp.skillName.toLowerCase() ||
                        comp.category.toLowerCase().includes(t.toLowerCase()),
                    )),
              );
              // Find matching SOP documents
              const recommendedDoc = documents.find(
                (d) =>
                  d.title
                    .toLowerCase()
                    .includes(comp.skillName.toLowerCase()) ||
                  (d.tags &&
                    d.tags.some((t) =>
                      comp.skillName.toLowerCase().includes(t.toLowerCase()),
                    )),
              );
              ...
              return (
                <div
                  key={comp.id}
                  className={`p-4 rounded-3xl border transition-all ${hasGap ? "border-amber-200 bg-amber-50/10" : "border-slate-100 bg-white"} space-y-3`}
                >
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <h3 className="font-extrabold text-[12.5px] text-slate-800 mt-0.5">
                        {comp.skillName}
                      </h3>
                    </div>
                    {hasGap ? (
                  ...
                  <div className="space-y-1.5">
                    <div className="flex justify-between text-[11px] font-bold text-slate-550">
                      ...
                      <span>
                        คาดหวังตามตำแหน่ง {currentUser.position}: L
                        {comp.expectedLevel}
                      </span>
                    </div>
                      ...
                      {[1, 2, 3, 4].map((idx) => {
                        ...
                        return (
                          <div key={idx} className="relative">
                            <div
                              className={`h-2 rounded-full ${cellBg} transition-colors`}
                            />
                            {isExpected && (
                  ...
                  <div className="pt-2 text-[10px] text-slate-500 italic leading-snug">
                    <strong className="text-slate-700 block not-italic font-bold font-mono">
                      ความหมายทักษะระดับ L{comp.actualLevel}:
                    </strong>
                    {comp.actualLevel === 1 &&
                      "รู้หลักการหรือเรียนทฤษฎี (Level 1)"}
                    {comp.actualLevel === 2 &&
                      "ทำตามคำชี้แนะได้มาตรฐาน (Level 2)"}
                    {comp.actualLevel === 3 &&
                  ...
                  {hasGap && (
                    <div className="p-2 bg-rose-50/20 border border-dashed border-rose-200 rounded-xl space-y-1.5 mt-2">
                      <p className="text-[10px] font-bold text-rose-800 flex items-center gap-1">
                        🚀 ปฏิบัติการแก้วิกฤตช่องว่างทักษะ (Skill Gap
                        Remediation)
                      </p>
                      <div className="flex flex-wrap gap-1.5 text-[9.5px]">
                        {recommendedCourse && (
                          <button
                            onClick={() => {
                              setActiveSubTab(
                                recommendedCourse.type === "Onboarding"
                                  ? "onboarding"
                                  : "general",
                              );
                              if (recommendedCourse.type === "Onboarding") {
                                setSelectedOnboardingCourseId(
                                  recommendedCourse.id,
                                );
                              } else {
                                setSelectedGeneralCourseId(
                                  recommendedCourse.id,
                                );
                              }
                              alert(
                                `🧭 นำทางคุณไปยังบทเรียนและหลักสูตรชำนาญการ: "${recommendedCourse.title}"`,
                              );
                            }}
                        ...
                        {recommendedDoc && (
                          <button
                            onClick={() => {
                              alert(
                                `🧭 กรุณาคลิก ค้นหา ด้านบนด้วยข้อควรอ้างอิง "${recommendedDoc.title}" เพื่อเปิดระบบ RAG คลี่คลายปัญหาอย่างถูกต้อง`,
                              );
                            }}
                        ...
                        {!recommendedCourse && !recommendedDoc && (
                          <span className="text-slate-400 italic">
                            กรุณาสอบถามศูนย์การเรียนรู้เรื่องคู่มือ ISO
                            งานเป่าฟิล์มโรงงานเพื่อขยับทักษะนี้
                          </span>
                        )}
                      </div>
                    </div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b pb-4 gap-4">
            <div>
              <span className="text-[10px] text-violet-600 font-bold uppercase tracking-widest block font-mono">
                AI PERSONALIZED ROADMAP PATHS //
                กำหนดกรอบเรียนรู้ส่วนบุคคลด้วยปัญญาประดิษฐ์
              </span>
              ...
              <p className="text-xs text-slate-450 mt-1">
                เรียนรู้วิถีขยายตัวในสายวิชาชีพของคุณ (Career Goal)
                วิเคราะห์โดยตรงร่วมฐานทักษะและคู่มือสิทธิ์ ISO
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                >
                  ...
                  <option value="Lead Quality Compliance Auditor (ดูแลระบบวิเคราะห์สิ่งแปลกปลอม ISO)">
                    🧪 ผู้ตรวจประเมินบริหารคุณภาพอาวุโส (ISO Lead QA/QC
                    Specialist)
                  </option>
                  <option value="WMS Automated Warehouse Commander (ดูแลคลังประตูดำอัจฉริยะ)">
                    📦 ตัวแทนควบคุมคลังสินค้าอัจฉริยะแบบกึ่งอัตโนมัติ (Digital
              ...
              >
                {aiLoading ? (
                  <>
                    <RefreshCw className="w-3.5 h-3.5 animate-spin" /> ค้นข้อมูล
                    & ประมวลผลจาก Gemini...
                  </>
                ) : (
                  <>
              ...
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-150 text-[10.5px] text-slate-500 leading-normal">
                <span className="font-bold text-slate-705 block mb-1">
                  ℹ️ วิธีการวิเคราะห์ของ Gemini AI:
                </span>
                โมเดลจะประเมินคะแนนสะสม โครงสร้างสมรรถนะของคุณในระบบ
                และคู่มือกระบวนการโรงงานเพื่อสกัดทางผ่าน Phase Roadmap
                ความเหมาะสมอย่างตรงไปตรงมา
              </div>
            </div>
              ...
              {aiLoading ? (
                <div className="flex-1 flex flex-col items-center justify-center text-center space-y-3 py-12">
                  ...
                  <p className="font-extrabold text-indigo-900 text-xs">
                    โมเดลกำลังวิเคราะห์คู่วิชา RMP
                    และขีดความสามารถสมรรถจริงเพื่อสร้าง Roadmaps...
                  </p>
                  <p className="text-[10px] text-slate-405 italic">
                    "Gemini is reading active catalog database for personal
                    alignments..."
                  </p>
                </div>
              ) : aiRoadmapResult ? (
                <div className="space-y-6 animate-fade-in text-left">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="relative border-l-2 border-slate-200 pl-5 ml-2 space-y-5">
                      {aiRoadmapResult.phases &&
                        aiRoadmapResult.phases.map((ph: any, phIdx: number) => (
                          <div key={phIdx} className="relative">
                            ...
                            <div>
                              <span className="text-[9.5px] text-violet-600 font-bold uppercase font-mono block">
                                Phase {phIdx + 1}: {ph.title}
                              </span>
                              <h4 className="font-black text-xs text-slate-800 mt-0.5">
                                {ph.milestoneGoal}
                              </h4>
                              ...
                              <div className="flex flex-wrap gap-2 pt-2.5">
                                {ph.coursesSuggested &&
                                  ph.coursesSuggested.map(
                                    (cs: string, csIdx: number) => {
                                      const matched = courses.find(
                                        (c) =>
                                          c.title
                                            .toLowerCase()
                                            .includes(cs.toLowerCase()) ||
                                          cs
                                            .toLowerCase()
                                        ...
                                        <button
                                          ...
                                          onClick={() => {
                                            if (matched) {
                                              ...
                                              alert(
                                                `📖 หลักสูตรเรียนรู้ภายนอก: "${cs}" (กรุณาส่งความประสงค์แก่แอดมินหรือหัวหน้าฝ่ายเพื่อลงทะเบียน)`,
                                              );
                                            }
                                          }}
                                ...
                                {ph.documentsSuggested &&
                                  ph.documentsSuggested.map(
                                    (ds: string, dsIdx: number) => (
                                      <button
                                        key={dsIdx}
                                        onClick={() =>
                                          alert(
                                            `🧭 โปรดเปิดคู่มือวิศวกรรม RAG Assistant ด้านหน้า แล้วระบุพินข้อความ "${ds}" เพื่อทำ RAG วินิจฉัยคู่มือ SOP ขององค์กร`,
                                          )
                                        }
              ...
              {attendanceLogs.length === 0 ? (
                ...
                <div className="overflow-x-auto border rounded-xl divide-y text-xs bg-slate-50/20">
                  {attendanceLogs.map((log: any) => (
                    <div
                      key={log.id}
                      className="p-3.5 bg-white flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:bg-slate-50 transition"
                    >
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] text-indigo-705 font-bold bg-indigo-50 px-1.5 py-0.2 rounded">
                            {getDepartmentById(log.department)?.name ||
                              log.department}
                          </span>
                        </div>
          ...
          <div className="lg:col-span-1 bg-white p-4 rounded-2xl border border-slate-205 space-y-4">
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 space-y-2 animate-pulse shadow-2xs">
                ...
                <div className="space-y-1.5 max-h-[160px] overflow-y-auto pr-1">
                  {pendingApprovals.map((c) => (
                    <div
                      key={c.id}
                      className="bg-white p-2 rounded-lg border border-amber-150 flex items-center justify-between gap-1.5 text-[10px]"
                    >
                      <div className="min-w-0 flex-1">
                        ...
                        <span className="text-[8.5px] text-slate-400 block">
                          รหัส: {c.id} | โดย: {c.createdByRole || "Editor"}
                        </span>
                      </div>
                      <button
                        onClick={(e) => {
            ...
            <div className="space-y-2">
              {(activeSubTab === "onboarding"
                ...
                const progObj = userProgressList.find(
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between gap-1 w-full">
                      <span className="leading-snug truncate max-w-[150px]">
                        {c.title}
                      </span>
                      {c.isApproved === false ? (
                        <span className="bg-amber-150 text-amber-950 border border-amber-250 text-[8px] px-1 rounded font-bold shrink-0 animate-pulse">
                          รออนุมัติ
                        </span>
                      ) : progObj?.status === "Completed" ? (
                        <span className="bg-green-150 text-green-900 text-[8.5px] px-1 rounded uppercase font-bold shrink-0">
                          Pass
                        </span>
                      ) : progObj?.status === "Learning" ? (
                        <span className="bg-amber-100 text-amber-900 text-[8.5px] px-1 rounded uppercase font-bold shrink-0">
                          Study
                        </span>
                      ) : null}
                    </div>
            ...
            {showCertificate && activeCertificateCourse ? (
              ...
              <div className="space-y-6 animate-fade-in text-center">
                ...
                {isOnboardingCert ? (
                  ...
                  <div className="bg-radial from-amber-50/20 to-orange-50/35 p-8 rounded-2xl border-4 border-double border-amber-300 shadow-inner max-w-xl mx-auto space-y-6 relative overflow-hidden text-slate-800">
                    ...
                    <div className="pt-2 text-center">
                      <p className="text-slate-500 text-[11px] italic">
                        ขอรับรองคุณวุฒิด้านความปลอดภัยและมาตรฐานปฏิบัติงานออนไซต์มอบให้สำหรับ:
                      </p>
                  ...
                  <div className="bg-radial from-[#fdfefd] via-[#f4fcfa] to-[#ffffff] p-8 rounded-2xl border-4 border-double border-teal-500 shadow-inner max-w-xl mx-auto space-y-6 relative overflow-hidden text-slate-800">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-[9px] text-slate-400 font-mono tracking-wider">
                        ROYAL MEIWA PAX CO., LTD. • QUALITY CONTROL & KNOWLEDGE
                        MANAGEMENT
                      </p>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 pt-6 text-[10px] border-t border-slate-100 text-slate-655 font-medium leading-normal">
                      <div className="text-left pl-3">
                        <p className="italic text-slate-400 font-serif">
                          ISO Quality Assurance
                        </p>
                        <p className="font-extrabold text-slate-800">
                          ระบบควบคุมประกันคุณภาพ
                        </p>
                      </div>
                      <div className="text-right pr-3 space-y-1">
                ...
                <div className="flex justify-center gap-3 text-xs font-semibold">
                  <button
                    onClick={() =>
                      alert(
                        "ดาวน์โหลดใบประกาศของวิชานี้สำเร็จ สำหรับใช้ยื่นจัดเกรด Competency Matrix ในระบบ ISO",
                      )
                    }
                ...
                <div>
                  <div className="flex justify-between items-center pb-2 border-b">
                    <span className="font-extrabold text-[#15329c] text-xs">
                      หัวข้อบทเรียนย่อยที่ {activeLessonIndex + 1} /{" "}
                      {lessons.length}
                    </span>
                  ...
                  <div className="bg-slate-50 p-6 rounded-2xl border border-slate-150 min-h-[300px] mt-4">
                    ...
                    {lessons[activeLessonIndex].mediaType &&
                      lessons[activeLessonIndex].mediaType !== "Text" && (
                          ...
                          <div className="bg-white p-4 flex flex-col items-center justify-center min-h-[220px]">
                            {lessons[activeLessonIndex].mediaType ===
                              ...
                              <div className="w-full">
                                {!lessons[activeLessonIndex].mediaUrl ? (
                                  <div className="w-full bg-slate-100 border-2 border-dashed border-slate-300 rounded-lg p-8 flex flex-col items-center justify-center text-center gap-2 h-52">
                                    <Film className="w-10 h-10 text-slate-350" />
                                    <span className="text-xs font-bold text-slate-500">
                                      ยังไม่มีวิดีโอสำหรับบทเรียนนี้
                                    </span>
                                    <span className="text-[10px] text-slate-400">
                                      กรุณาแนบไฟล์หรือลิงก์วิดีโอที่หน้าจัดการคอร์ส
                                    </span>
                                  </div>
                                ) : lessons[
                                    activeLessonIndex
              ...
              <div className="space-y-6 flex-1 flex flex-col justify-between">
                <div>
                  <div className="pb-3 border-b border-slate-150 flex items-center justify-between">
                    <div>
                      <span className="bg-rose-100 text-rose-800 px-2 py-0.5 rounded text-[9px] font-black uppercase">
                        Quiz mode
                      </span>
                      <h3 className="font-extrabold text-slate-800 text-sm mt-1">
                        แบบทดสอบวัดระดับทักษะ: {selectedCourse?.title}
                      </h3>
                    </div>
                    <button
                  ...
                  {quizScore === null ? (
                    ...
                    <form
                      ...
                      className="space-y-6 mt-4 text-xs"
                    >
                      ...
                      {questions.map((q, qIndex) => {
                        ...
                        return (
                          <div
                            key={q.id}
                            className="space-y-3 p-4 bg-slate-50/50 rounded-xl border border-slate-100"
                          >
                            <div className="flex items-center justify-between">
                                ...
                                {q.options.map((opt, oIdx) => (
                                  ...
                                  >
                                    <input
                                      ...
                                      onChange={() =>
                                        handleQuizAnswerSelect(q.id, opt)
                                      }
                              ...
                              <div className="space-y-2 bg-white p-3 rounded-lg border border-slate-100">
                                ...
                                {q.pairs.map((pair, pIdx) => {
                                  ...
                                  return (
                                    <div
                                      key={pIdx}
                                      className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 p-2 rounded-lg bg-slate-50 border border-slate-100 text-xs"
                                    >
                        ...
                        {quizPassed ? (
                          <div className="flex flex-wrap items-center justify-center gap-2.5">
                            ...
                            >
                              ...
                              <span>
                                เปิดเคลมเหรียญตรา & ใบวิชาชีพดิจิทัล (Claim
                                Badge & Cert)
                              </span>
                            </button>
              ...
              >
                <div className="space-y-4">
                  ...
                  <div className="pb-3 border-b border-slate-150">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {selectedCourse.targetPositions.map((pos) => (
                        <span
                          key={pos}
                          className="bg-slate-105 text-slate-600 text-[9px] font-bold px-1.5 py-0.5 rounded-full border"
                        >
                          🎯 กลุ่มเป้าหมาย: {pos}
                        </span>
                      ))}
                  ...
                  {selectedCourse.isApproved === false && (
                    <div className="bg-amber-50 border border-amber-200/80 rounded-xl p-3.5 text-xs text-amber-900 flex items-start gap-2.5 animate-pulse">
                      ...
                      <div>
                        <span className="font-extrabold block text-[11.5px] text-amber-950">
                          หลักสูตรนี้อยู่ในสถานะ "รออนุมัติ" (Pending Approval)
                        </span>
                  ...
                  <div className="space-y-2.5 pt-2">
                    <h4 className="font-extrabold text-xs text-slate-400 uppercase tracking-wider block mb-1">
                      📚 รายละเอียดบทเรียนในคาบหลักสูตร ({lessons.length}{" "}
                      บทเรียน):
                    </h4>
                    ...
                    <div className="grid grid-cols-1 gap-2 border rounded-xl divide-y text-xs bg-slate-50/20 max-h-[300px] overflow-y-auto">
                      {lessons.map((less, idx) => {
                        const isLearning =
                          ...
                          idx <= completedLessonsCount;
                        return (
                          <div
                            key={less.id}
                            className="p-3 flex items-center justify-between gap-3 bg-white hover:bg-slate-50 transition"
                          >
                            <div className="flex items-center gap-2">
                ...
                <div className="border-t border-slate-150 pt-4 mt-8 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs font-bold bg-[#fbfbf9]/50 p-2.5 rounded-xl">
                  <div className="text-slate-505 font-mono text-[10.5px]">
                    {relevantResult
                      ? "ระดับหลักสูตร: ได้รับการอนุมัติ (Certified)"
                      : "ยังไม่เคยประเมินหลักสูตรนี้"}
                  </div>
                  ...
                  <div className="flex gap-2 w-full sm:w-auto">
                      ...
                      (currentUser.role === "Admin" ||
                        ...
                        <button
                          onClick={() => {
                            setEditingCourse({
                              ...selectedCourse,
                              targetPositionsStr: selectedCourse.targetPositions
                                ? selectedCourse.targetPositions.join(", ")
                                : "",
                              lessonsStr: JSON.stringify(
                                selectedCourse.lessons,
                                null,
                                2,
                              ),
                              quizStr: JSON.stringify(
                                selectedCourse.quiz,
                                null,
                                2,
                              ),
                    ...
                    {currentUser && currentUser.role === "Admin" && (
                      <button
                        onClick={() => {
                          if (
                            confirm(
                              "คุณแน่ใจว่าต้องการลบหลักสูตรนี้ออกจากฐานสมองและระบบ Audit หรือไม่?",
                            )
                          ) {
                    ...
                    {selectedCourse.isApproved === false ? (
                      currentUser?.role === "Admin" ? (
                        <button
                          onClick={() => {
                            ...
                            alert(
                              `✅ อนุมัติหลักสูตร "${selectedCourse.title}" เรียบร้อยแล้ว!`,
                            );
                          }}
                        ...
                        <div className="bg-amber-100 text-amber-905 border border-amber-250 rounded-xl px-4 py-2.5 text-center flex-1 font-bold text-[10.5px] flex items-center justify-center gap-1.5">
                          ...
                          อยู่ระหว่างรอ Admin อนุมัติวิชา
                        </div>
                      )
                    ) : (
                      <button
                      ...
                      >
                        ...
                        {relevantResult
                          ? "กลับไปอ่านทบทวนวิชา (Review)"
                          : "คลิกเริ่มเข้าเรียนวิชาทฤษฎี (Start)"}
                      </button>
                    )}
                  </div>
                </div>
              ...
              <div className="p-8 text-center bg-slate-50/50 border-2 border-dashed rounded-3xl shrink-0 my-auto text-slate-400">
                ...
                <p className="text-xs font-bold mt-2">
                  ยังไม่มีหลักสูตรบรรจุในหมวดหมู่นี้
                </p>
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-3xl shadow-xl border w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="bg-gradient-to-r from-indigo-600 to-indigo-750 p-4 font-bold text-white text-sm shrink-0 flex items-center justify-between">
              <span className="flex items-center gap-1.5">
                <Plus className="w-4 h-4" /> สถาบันความรู้:
                เพิ่มหลักสูตรและกำหนดสื่อการสอนใหม่
              </span>
              <button
                type="button"
            ...
            <form
              ...
              className="p-6 overflow-y-auto space-y-6 text-xs flex-1"
            >
              ...
              <div className="space-y-3.5">
                  ...
                  <div className="space-y-1">
                    ...
                    >
                      <option value="Onboarding">
                        🔰 ออนบอร์ดพนักงานใหม่ (Onboarding)
                      </option>
                      <option value="General">
                        🎯 วิชาเสริมศักยภาพช่างเทคนิค (General)
                      </option>
                    </select>
                  </div>
                ...
                <div className="space-y-1.5">
                  <label className="font-bold text-slate-700 block">
                    กลุ่มพนักงานเป้าหมาย (ระดับประเมินความสามารถ):*
                  </label>
                    ...
                    {[
                      {
                        id: "pos-qc",
                        val: "QC",
                        label: "🔬 พนักงานประกันคุณภาพ (QC/QA)",
                      },
                      {
                        id: "pos-wh",
                        val: "Warehouse Staff",
                        label: "📦 พนักงานคลังสินค้า (Warehouse)",
                      },
                      {
                        id: "pos-op",
                        val: "Operator",
                        label: "⚙️ พนักงานเดินสายการผลิต (Operator)",
                      },
                      {
                        id: "pos-tc",
                        val: "Technician",
                        label: "🛠️ แผนกช่างซ่อมบำรุง (Technician)",
                      },
                      {
                        id: "pos-all",
                        val: "All Staff",
                        label: "👥 พนักงานทั่วไป (All Staff)",
                      },
                    ].map((item) => {
                      const isChecked = newCourseState.targetPositions.includes(
                        item.val,
                      );
                      return (
                        <label
                          key={item.id}
                          className="flex items-center gap-2 cursor-pointer hover:text-indigo-700 select-none text-[11px]"
                        >
                          <input
                            type="checkbox"
                            checked={isChecked}
                            onChange={() => {
                              if (isChecked) {
                                setNewCourseState({
                                  ...newCourseState,
                                  targetPositions:
                                    newCourseState.targetPositions.filter(
                                      (p) => p !== item.val,
                                    ),
                                });
                              } else {
              ...
              <div className="space-y-4">
                <h4 className="font-extrabold text-[#15329c] text-xs border-b pb-1.5 flex items-center gap-1.5">
                  📂 ส่วนที่ 2: เนื้อหาบทเรียนและสื่อทัศนูปกรณ์ (Learning
                  Material / Media)
                </h4>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setNewCourseState({
                          ...newCourseState,
                          lessonMediaType: e.target.value as any,
                        })
                      }
                      className="w-full bg-slate-50 border border-slate-200 p-2.5 rounded-lg text-xs text-slate-700 font-bold"
                    >
                      <option value="PDF">
                        📄 เอกสารฉบับคู่มือสากล (PDF File)
                      </option>
                      <option value="Image">
                        📷 รูปแผนภูมิขั้นตอนหน้างาน (SOP Diagram Image)
                      </option>
                      <option value="Slides">
                        📊 งานนำเสนอพรีเซนเทชัน (PowerPoint Slides)
                      </option>
                      <option value="Video">
                        🎥 วิดีโอสตรีมขั้นตอนปฏิบัติงานจริง (MP4 / YouTube
                        Video)
                      </option>
                      <option value="Text">
                        📝 บทเขียนวิเคราะห์และเอกสารประกอบ (Plain Text Only)
                      </option>
                    </select>
                  </div>
                </div>
                      ...
                      {newCourseState.isSimulatedUploading ? (
                        <div className="py-2 flex flex-col items-center justify-center space-y-2.5">
                          ...
                          <span className="text-[10px] text-indigo-700 font-semibold">
                            กำลังตรวจสอบไฟล์และจัดเตรียมลิงก์เข้าสารระบบ...
                          </span>
                        </div>
                      ) : (
                        <div className="space-y-1.5">
                          ...
                          />
                          <GraduationCap
                            className="w-10 h-10 text-indigo-500/80 mx-auto"
                            strokeWidth={1.5}
                          />
                          <div className="text-slate-700 font-bold text-[11px]">
                            {newCourseState.simulatedFileName ? (
                              ...
                              <span>
                                ลากและวางไฟล์ {newCourseState.lessonMediaType}{" "}
                                หรือ คลิกเพื่ออัปโหลด
                              </span>
                            )}
                          </div>
                          ...
                          <button
                            ...
                            onClick={() =>
                              document
                                .getElementById("course-media-file-native")
                                ?.click()
                            }
              ...
              <div className="space-y-4">
                <h4 className="font-extrabold text-[#15329c] text-xs border-b pb-1.5 flex items-center gap-1.5">
                  🧪 ส่วนที่ 3: ข้อสอบวัดสัมฤทธิ์ปลายหลักสูตร (Competency
                  Assessment Question)
                </h4>
                ...
                {addedQuizQuestions.length > 0 && (
                  <div className="bg-indigo-50/40 border border-indigo-150 rounded-xl p-3 space-y-2 animate-fade-in">
                    <span className="font-extrabold text-indigo-950 block text-[11px] flex items-center gap-1">
                      📝 รายการคำถามที่เพิ่มสำเร็จแล้ว (
                      {addedQuizQuestions.length} ข้อ):
                    </span>
                    <div className="space-y-1.5 max-h-[140px] overflow-y-auto pr-1">
                      {addedQuizQuestions.map((q, idx) => (
                        <div
                          key={q.id}
                          className="bg-white p-2.5 rounded-lg border border-indigo-100 flex items-start justify-between gap-2.5 text-[10px]"
                        >
                          <div className="min-w-0 flex-1">
                            <span className="font-bold text-slate-800 block">
                            ...
                            <span className="text-[9px] text-emerald-700 font-bold block mt-0.5">
                              ✓ เฉลย:{" "}
                              {q.type === "Matching"
                                ? "จับคู่ถูกต้องครบถ้วนตามความสัมพันธ์"
                                : q.correctAnswer}
                            </span>
                          </div>
                          <button
                ...
                <div className="space-y-2 bg-slate-50/50 border border-slate-150 p-4 rounded-xl space-y-3">
                  <div className="flex justify-between items-center">
                    <span className="font-bold text-[#15329c] text-[11px]">
                      {addedQuizQuestions.length > 0
                        ? `➕ ร่างข้อสอบข้อที่ ${addedQuizQuestions.length + 1}`
                        : "✍️ ป้อนข้อสอบข้อแรก"}
                    </span>
                    <span className="text-[9.5px] text-slate-400">
                      เลือกประเภทและป้อนคำถาม
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-1">
                      <label className="font-semibold text-slate-600 block">
                        ประเภทคำถามข้อสอบ (Question Type):*
                      </label>
                      <select
                      ...
                      >
                        <option value="SingleChoice">
                          Multiple Choice (คำถามปรนัยหลายตัวเลือก)
                        </option>
                        <option value="TrueFalse">
                          True / False (แบบถูก-ผิด)
                        </option>
                    ...
                    <div className="space-y-1">
                      <label className="font-semibold text-slate-600 block">
                        โจทย์คำถามทดสอบวัดและประเมินผล:*
                      </label>
                      <input
                        type="text"
                        ...
                        placeholder={
                          newCourseState.quizType === "TrueFalse"
                            ? "เช่น การไม่สวมหมวกนิรภัยในพื้นที่ผลิตถือเป็นเรื่องยอมรับได้หรือไม่?"
                            : newCourseState.quizType === "Matching"
                              ? "จงจับคู่อุปกรณ์ PPE ให้ตรงกับหน้าที่ที่ถูกต้อง"
                              : newCourseState.quizType === "Essay"
                                ? "จงอธิบายขั้นตอนการทำความสะอาดพื้นที่แบบ 5ส ด้วยตนเอง"
                  ...
                  {(newCourseState.quizType || "SingleChoice") ===
                    "SingleChoice" && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 animate-fade-in">
                      <div className="space-y-1">
                        <div className="flex justify-between items-center">
                          <label className="font-semibold text-slate-600 block">
                            ตัวเลือกคำตอบข้อสอบ (1 ตัวเลือกต่อบรรทัด):*
                          </label>
                          <span className="text-[10px] text-slate-400">
                            แนะนำ 3-4 ตัวเลือก
                          </span>
                      ...
                      <div className="space-y-1">
                        <label className="font-semibold text-slate-600 block">
                          คำตอบที่ถูกต้องที่สุด (ต้องตรงกับตัวเลือกข้างต้น):*
                        </label>
                        <input
                          type="text"
                          required={addedQuizQuestions.length === 0}
                          ...
                          onChange={(e) =>
                            setNewCourseState({
                              ...newCourseState,
                              quizCorrectAnswer: e.target.value,
                            })
                          }
                          placeholder="ล้างมือทำความสะอาดร่างกาย"
                  ...
                  {(newCourseState.quizType || "SingleChoice") ===
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 animate-fade-in">
                      ...
                      <div className="space-y-1">
                        ...
                        <select
                          value={
                            newCourseState.quizCorrectAnswer || "ถูก (True)"
                          }
                          onChange={(e) =>
                            setNewCourseState({
                              ...newCourseState,
                  ...
                  {(newCourseState.quizType || "SingleChoice") ===
                    ...
                    <div className="grid grid-cols-1 gap-3 animate-fade-in">
                      <div className="space-y-1">
                        <div className="flex justify-between items-center">
                          <label className="font-semibold text-slate-600 block">
                            คู่คำตอบฝั่งซ้ายและฝั่งขวา (รูปแบบ: ซ้าย = ขวา, 1
                            คู่ต่อบรรทัด):*
                          </label>
                          <span className="text-[10px] text-amber-600 font-semibold">
                            ⚠️ ระบบจะจัดคู่เฉลยและสลับตัวเลือกตัวจับคู่อัตโนมัติ
                          </span>
                        </div>
                        <textarea
                          required={addedQuizQuestions.length === 0}
                  ...
                  {(newCourseState.quizType || "SingleChoice") === "Essay" && (
                    <div className="grid grid-cols-1 gap-3 animate-fade-in">
                      <div className="space-y-1">
                        <label className="font-semibold text-slate-600 block">
                          เกณฑ์การพิจารณาหรือคีย์เวิร์ดเฉลย (ไม่บังคับ):
                        </label>
                        <input
                          type="text"
                          value={newCourseState.quizCorrectAnswer}
          ...
          <div className="bg-white rounded-3xl shadow-xl border w-full max-w-xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            >
              ...
              <div className="space-y-1">
                <div className="flex justify-between items-center">
                  <label className="font-semibold text-slate-600 block">
                    บทเรียน (Lessons Structure JSON):
                  </label>
                  <span className="text-[10px] text-indigo-700 italic">
                    ฟอร์แมต JSON แอรเรย์
                  </span>
                </div>
                <textarea
                  rows={5}
              ...
              <div className="space-y-1">
                <div className="flex justify-between items-center">
                  <label className="font-semibold text-slate-600 block">
                    ข้อสอบแนววิเคราะห์ท้ายวิชา (Quiz Questions JSON):
                  </label>
                  <span className="text-[10px] text-[#e51a24] italic">
                    ระเบียบคำตอบ correctAnswer
                  </span>
                </div>
                <textarea
                  rows={4}
