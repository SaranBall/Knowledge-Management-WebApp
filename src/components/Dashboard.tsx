...
export const Dashboard: React.FC<DashboardProps> = ({
  ...
  const activeTranscriptCompletions = getTranscriptCompletedCourses(
  ...
  const handlePrint = () => {
    const printContents =
      document.getElementById("printable-iso-area")?.innerHTML;
    if (printContents) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-4">
            ...
            <div>
              ...
              <p className="text-slate-500 text-xs mt-1">
                องค์ความรู้และมาตรฐานอนุมัติใช้
              </p>
            </div>
            <div className="pt-2 border-t border-slate-100 grid grid-cols-3 gap-2 text-center">
              <div>
                <span className="text-[#e51a24] font-bold text-sm">
                  +{newKnowledgeThisMonth}
                </span>
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-4">
            ...
            <div>
              ...
              <p className="text-slate-500 text-xs mt-1">
                ได้รับการแต่งตั้งครอบคลุมทุกแผนก
              </p>
            </div>
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between space-y-4">
            ...
            <div>
              <div className="text-3xl font-black text-slate-800 tracking-tight">
                {searchLogs.length > 0
                  ? Math.round(
                      (searchLogs.filter((s) => s.hasResult).length /
                        searchLogs.length) *
                        100,
                    )
                  : 91}
              ...
              <p className="text-slate-500 text-[11px] mt-1">
                อัตราตอบคำถามสืบค้นอัจฉริยะสำเร็จ (Resolution Rate)
              </p>
            </div>
            <div className="space-y-1.5 pt-2">
              ...
              <div className="space-y-3 my-3">
                {gapAnalysisList.length === 0 ? (
                  ...
                  gapAnalysisList.map((gap, i) => (
                    <div
                      key={i}
                      className="flex items-center justify-between p-3 bg-red-50/50 hover:bg-neutral-50 rounded-xl border border-red-100 transition"
                    >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h3 className="font-extrabold text-slate-900 text-sm">
                      เกณฑ์คะแนนเฉลี่ยคอร์สฝึกอบรมหลัก
                    </h3>
                    <p className="text-xs text-slate-400">
                      คะแนนเฉลี่ยจากการทำแบบทดสอบแยกหลักสูตร
                    </p>
                  </div>
                </div>
              </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
          <div className="flex items-center justify-between pb-3 border-b border-slate-100 mb-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="font-bold text-slate-900 text-sm">
                  Competency Matrix & Learning Transcripts
                </h3>
                <p className="text-xs text-slate-500">
                  ติดตามพนักงานแต่ละท่านว่าอบรมครบตามกำหนดของตำแหน่งงานหรือไม่
                </p>
              </div>
            </div>
          </div>
              ...
              <div className="md:col-span-2 space-y-4">
                <div className="flex items-start justify-between gap-4 flex-wrap pb-2 border-b border-slate-100">
                  <div>
                    <h4 className="font-black text-slate-800 text-sm tracking-tight flex items-center gap-1.5">
                      Learning Transcript การฝึกอบรม:{" "}
                      {selectedUserTranscript.name}
                    </h4>
                  ...
                  <div className="space-y-2">
                    {getRequiredCoursesForPosition(
                      ...
                      const currentProgress = userProgressList.find(
                        (p) =>
                          p.userId === selectedUserTranscript.id &&
                          p.courseId === reqCourseId,
                      );
                      const examResultObj = examResults.find(
                        (e) =>
                          e.employeeId === selectedUserTranscript.employeeId &&
                          e.courseId === reqCourseId,
                      );

                      const isCompleted =
                        currentProgress?.status === "Completed" ||
                        (examResultObj && examResultObj.pass);
                      const scoreVal =
                        examResultObj?.score ?? currentProgress?.score;
                ...
                <div>
                  <span className="block text-[10px] font-bold text-slate-400 uppercase mb-2">
                    ประวัติการเรียนและการรับใบวิชาชีพดิจิทัล (Digital Badge &
                    Certificate Logs)
                  </span>
                  <div className="border border-slate-150 rounded-xl overflow-hidden text-xs bg-white">
                    ...
                    {activeTranscriptCompletions.map((item, idx) => (
                      <div
                        ...
                        className="p-2.5 grid grid-cols-12 border-t border-slate-100 hover:bg-slate-50/50 items-center transition"
                      >
            ...
            <div className="bg-[#15329c] text-white px-6 py-4 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-extrabold text-xs tracking-tight">
                  ISO Audit Compliance Report PDF Preview
                </h3>
              </div>
              <button
            ...
            <div className="flex-1 p-6 overflow-y-auto bg-slate-50">
              <div className="mb-4 text-center text-xs text-slate-600 bg-amber-50 border border-amber-200 p-3 rounded-xl leading-relaxed">
                <span>
                  💡{" "}
                  <strong>
                    คำแนะนำการประเมิน ISO 9001:2015 Clause 7.2 (Competence):
                  </strong>{" "}
              ...
              >
                ...
                <div className="pb-6 border-b-2 border-[#15329c]/30 flex flex-col md:flex-row items-center justify-between gap-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h4 className="font-black text-slate-900 text-sm tracking-wider">
                        ROYAL MEIWA PAX CO., LTD.
                      </h4>
                      <p className="text-[10px] text-slate-500 mt-0.5">
                        บริษัท รอแยล เมอิวะ แพ็คซ์ จำกัด
                        (สำนักงานพัฒนามาตรฐานวิศวกรรม & HRD)
                      </p>
                      <p className="text-[9px] text-slate-400 font-mono">
                        ISO 9001:2015 REGISTERED FACILITY • FACTORY SEC
                        SUKHUMVIT 74
                      </p>
                    </div>
                  </div>
                  <div className="text-right border-l-0 md:border-l border-slate-205 pl-0 md:pl-4 space-y-1">
                    ...
                    <p className="text-[10px] text-slate-500 font-mono">
                      REF No: RMP-TR-ISO-{selectedUserTranscript.employeeId}
                    </p>
                    <p className="text-[9px] text-slate-400 font-mono">
                      Rev. 02 • Effective: 2026-06-16
                    </p>
                  </div>
                </div>
                ...
                <div className="py-6 text-center">
                  ...
                  <h4 className="font-mono text-slate-500 text-[10px] mt-1 font-bold">
                    INDIVIDUAL TRAINING TRANSCRIPT & COMPETENCY ASSESSMENT
                    REPORT
                  </h4>
                </div>
                ...
                <div className="space-y-3 mb-6 text-left">
                  <div className="border-b border-[#15329c]/20 pb-1.5 flex items-center justify-between">
                    <span className="text-[10.5px] font-black text-[#15329c] tracking-wider uppercase">
                      ส่วนที่ 1: ตารางประเมินระดับสมรรถนะขั้นบังคับ (COMPETENCY
                      MATRIX EVALUATION)
                    </span>
                    <span className="text-[9px] text-slate-400 font-mono">
                      ISO 9001:2015 Clause 7.2 Requirements
                    </span>
                  </div>
                  ...
                  <div className="space-y-2 text-xs">
                    {getRequiredCoursesForPosition(
                      ...
                      return (
                        <div
                          key={reqCourseId}
                          className="flex flex-col sm:flex-row sm:items-center justify-between p-3 bg-slate-50 rounded-lg border border-slate-150 gap-2"
                        >
                          <div>
                ...
                <div className="space-y-3 mb-6 text-left">
                  <div className="border-b border-[#15329c]/20 pb-1.5 flex items-center justify-between">
                    <span className="text-[10.5px] font-black text-[#15329c] tracking-wider uppercase">
                      ส่วนที่ 2:
                      บันทึกข้อมูลและชั่วโมงสะสมการอบรมหลักสูตรออนไลน์ (ONLINE
                      LEARNING LOGS)
                    </span>
                    <span className="text-[9px] text-slate-400 font-mono">
                      E-Learning & Interactive Core Sessions
                    </span>
                  </div>
                    ...
                    {getUserBadges(
                      ...
                      getUserBadges(
                        ...
                        .map((badge) => (
                          <div
                            key={badge.id}
                            className="bg-white border border-slate-200 p-2.5 rounded-xl flex items-center gap-2 shadow-xs transition"
                          >
                ...
                <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6 pt-6 border-t border-slate-200 text-center text-xs">
                  ...
                  <div className="flex flex-col items-center justify-center p-3 border border-dashed border-emerald-500/30 bg-emerald-50/40 rounded-xl space-y-1.5">
                    ...
                    <strong className="text-emerald-800 text-[10px] font-black tracking-wider uppercase">
                      QA & SHE SYSTEM VERIFIED
                    </strong>
