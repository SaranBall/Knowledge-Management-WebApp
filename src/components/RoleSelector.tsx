...
export const RoleSelector: React.FC<RoleSelectorProps> = ({
  ...
  return (
    <div
      id="simulated-role-panel"
      className="bg-slate-900 text-slate-100 border-b border-indigo-950 px-3 py-2.5 sm:px-6"
    >
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400">
              คลิกสลับบัญชีเพื่อทดสอบสิทธิ์ (Admin / Editor / Viewer)
              และการทำงานแต่ละเวิร์กโฟลว์
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          {users.map((user) => {
            ...
            if (displayedRole === "Admin")
              roleBadgeClass = isSelected
                ? "bg-red-500 text-white"
                : "bg-red-500/20 text-red-300";
            if (displayedRole === "Editor")
              roleBadgeClass = isSelected
                ? "bg-amber-500 text-slate-900"
                : "bg-amber-500/20 text-amber-300";
            if (displayedRole === "Viewer")
              roleBadgeClass = isSelected
              ...
              >
                ...
                <div>
                  <div className="font-medium text-slate-200 flex items-center gap-1.5 leading-none">
                    ...
                    <span
                      className={`text-[9px] px-1 py-0.2 rounded font-mono ${roleBadgeClass}`}
                    >
                      {displayedRole}
                    </span>
                  </div>
                ...
                {isSelected && (
                  <UserCheck className="w-3.5 h-3.5 text-indigo-400 ml-1 shrink-0" />
                )}
              </button>
            );
          })}
