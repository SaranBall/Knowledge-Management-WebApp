...
export const MemberManagement: React.FC<MemberManagementProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<
    "MEMBERS" | "REPORTS" | "EMPLOYEE_MASTER" | "AUDIT_LOGS"
  >("MEMBERS");
  ...
  const handleEditUserSubmit = (e: React.FormEvent) => {
    ...
    const duplicate = users.some(
      (u) =>
        u.id !== editForm.id &&
        u.employeeId.toLowerCase() === editForm.employeeId.toLowerCase(),
    );
    if (duplicate) {
  ...
  const handleNewUserAvatarChange = async (
  ...
  const handleEditUserAvatarChange = async (
    e: React.ChangeEvent<HTMLInputElement>,
  ) => {
    const file = e.target.files?.[0];
    if (file) {
  ...
  const mapHeaders = (row: any[]) => {
    ...
    row.forEach((cell, idx) => {
      ...
      if (
        str.includes("employeeid") ||
        str.includes("employee id") ||
        str.includes("รหัสพนักงาน") ||
        str.includes("รหัส") ||
        str.includes("id")
      ) {
        mapping.employeeId = idx;
      } else if (
        str.includes("fullname") ||
        str.includes("name") ||
        str.includes("ชื่อ") ||
        str.includes("นามสกุล") ||
        str.includes("ชื่อ-นามสกุล")
      ) {
        mapping.name = idx;
      } else if (
        ...
        str.includes("startdate") ||
        str.includes("start date") ||
        str.includes("วันเริ่มงาน") ||
        str.includes("วันที่เริ่มงาน")
      ) {
    ...
    if (
      ...
      reader.onload = (e) => {
        try {
          ...
          if (json.length < 2) {
            throw new Error(
              "ไฟล์ Excel ไม่มีข้อมูลเพียงพอ หรือไม่มีแถวส่วนหัวหลัก",
            );
          }
          ...
          for (let i = 1; i < json.length; i++) {
            ...
            // Extract values
            const empIdVal =
              mapping.employeeId !== undefined
                ? row[mapping.employeeId]
                : row[0];
            const nameVal =
              mapping.name !== undefined ? row[mapping.name] : row[1];
            const deptVal =
            ...
            const startVal =
              mapping.startDate !== undefined ? row[mapping.startDate] : row[4];
            const lvlVal =
              mapping.level !== undefined ? row[mapping.level] : row[5];
            const emailVal =
              mapping.email !== undefined ? row[mapping.email] : row[6];
            const phoneVal =
              mapping.phone !== undefined ? row[mapping.phone] : row[7];
          ...
          if (list.length === 0) {
            throw new Error(
              "ไม่มีข้อมูลพนักงานที่สามารถแปลงค่าได้ (โปรดตรวจสอบคอลัมน์ รหัสพนักงาน และ ชื่อพนักงาน)",
            );
          }

          setParsedEmployees(list);
          setParserSuccessMsg(
            `สแกนไฟล์ Excel/CSV สำเร็จ: พบข้อมูลพนักงานทั้งหมด ${list.length} แถว`,
          );
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(
            `เซิร์ฟเวอร์ส่งกลับข้อผิดพลาดสถานะ ${response.status}`,
          );
        }
        ...
        if (result.success && result.employees && result.employees.length > 0) {
          ...
          setParserSuccessMsg(
            `วิเคราะห์เอกสาร PDF/ภาพแสกนด้วย AI สำเร็จ: พบข้อมูลพนักงาน ${result.employees.length} แถว`,
          );
          setShowImportPreview(true);
        } else {
          throw new Error(
            result.message ||
              "โมเดล AI ไม่สามารถถอดโครงสร้างข้อมูลพนักงานจากเอกสารนี้ได้",
          );
        }
      } catch (err: any) {
        setParserError(
          err.message || "เกิดข้อผิดพลาดในการเรียกใช้ AI Parser บนเซิร์ฟเวอร์",
        );
      } finally {
        setParserLoading(false);
      }
      ...
      setParserError(
        "ไม่สนับสนุนรูปแบบไฟล์นี้ กรุณาอัปโหลด .xlsx, .xls, .csv, หรือ .pdf เท่านั้น",
      );
      setParserLoading(false);
    }
  };
  ...
  const [reportType, setReportType] = useState<
    "INDIVIDUAL" | "DEPARTMENT" | "ALL" | "SEARCH_LOGS"
  >("ALL");
  const [selectedRepUser, setSelectedRepUser] = useState<string>(
    users[0]?.id || "",
  );
  ...
  const filteredUsers = visibleUsers.filter((u) => {
    const deptName = getDepartmentById(u.departmentId)?.name || "";
    const matchStr =
      `${u.name} ${u.employeeId} ${deptName} ${u.position} ${u.role}`.toLowerCase();
  ...
  const handleExportCsv = (
    ...
    if (currentUser.role === "Viewer" && scope !== "INDIVIDUAL_CSV") {
      alert(
        "❌ สิทธิ์เข้าชมทั่วไป (Viewer) ของคุณสามารถดาวน์โหลดเฉพาะรายงานผลการเรียนรู้ของตัวเองได้เท่านั้น",
      );
      return;
    }
    if (
      ...
      (scope === "ALL" ||
        scope === "ISO_AUDIT_AGGREGATED" ||
        scope === "RAW_SEARCH_LOGS")
    ) {
      alert(
        "❌ สิทธิ์หัวหน้างาน (Editor) ของคุณสามารถดาวน์โหลดเฉพาะรายงานแผนกตนเองได้เท่านั้น (ระดับองค์กรจำกัดสิทธิ์เฉพาะ Admin)",
      );
      return;
    }
    ...
    if (scope === "ALL" || scope === "DEPARTMENT") {
      ...
      if (scope === "DEPARTMENT") {
        const isMain = getDepartmentById(selectedRepDeptId)?.parentId === null;
        targetUsers = isMain
          ? targetUsers.filter(
              (u) =>
                getMainDepartmentOf(u.departmentId)?.id === selectedRepDeptId,
            )
          : targetUsers.filter((u) => u.departmentId === selectedRepDeptId);
        const deptCode = getDepartmentById(selectedRepDeptId)?.code || "DEPT";
        filename = `learning_report_${deptCode}_${new Date().toISOString().split("T")[0]}.csv`;
      } else {
      ...
      const section1Header = [
        "=== ส่วนที่ 1: รายงานสถิติการฝึกอบรมและประเมินผลพนักงาน (Part 1: Employee Training & Competency Audit) ===",
      ];
      const headers1 = [
        "รหัสพนักงาน (Employee ID)",
        "ชื่อ-นามสกุลจริง (Full Name)",
      ...
      const section2Header = [
        "",
        "",
        "=== ส่วนที่ 2: รายงานการวิเคราะห์ความพยายามสืบค้นและช่องว่างความรู้เพื่อปรับปรุงอย่างต่อเนื่อง (Part 2: Continuous Improvement & Knowledge Gaps Audit - ISO Clause 10) ===",
      ];
      const headers2 = [
      ...
      const rows = searchLogs.map((log) => {
        ...
        const dept = matchingUser
          ? getDepartmentById(matchingUser.departmentId)?.name || "N/A"
          : "N/A";
        const empId = matchingUser
          ...
          : "N/A";
        const isUnresolvedGap =
          !log.hasResult && !resolvedGaps.includes(log.keyword);
      ...
      if (
        ...
        alert(
          `❌ สิทธิ์ของคุณเข้าถึงได้เฉพาะพนักงานสังกัดแผนก ${getDepartmentById(currentUser.departmentId)?.name || ""} เท่านั้น`,
        );
        return;
      }
      ...
      const userCompletedProgresses = userProgressList.filter(
        (p) => p.userId === u.id,
      );
      const userExamLogs = examResults.filter(
        (e) => e.employeeId === u.employeeId,
      );

      const rows = reqCourseIds.map((courseId) => {
        const courseObj = courses.find((c) => c.id === courseId);
        const progressObj = userCompletedProgresses.find(
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white p-4 rounded-xl border border-slate-200">
            ...
            {currentUser.role === "Admin" ? (
              ...
              <span className="text-[10.5px] font-mono italic text-slate-400">
                เฉพาะแอดมินกำหนดสิทธิ์และเพิ่มสมาชิกเท่านั้น
              </span>
            )}
          </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden text-xs">
            ...
            <div className="grid grid-cols-2 md:grid-cols-12 p-4 bg-slate-50/80 border-b border-slate-200 font-bold text-slate-650 tracking-wider">
              ...
              <div className="md:col-span-1 text-right font-bold text-slate-600">
                จัดการ
              </div>
            </div>
            ...
            <div className="divide-y divide-slate-150">
              {filteredUsers.map((u) => {
                ...
                return (
                  <div
                    key={u.id}
                    className="grid grid-cols-2 md:grid-cols-12 p-4 items-center hover:bg-slate-50/50 transition gap-y-3"
                  >
                    {/* Name column */}
                    <div className="md:col-span-3 flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[10.5px] text-slate-400 font-mono mt-0.5">
                          ID: <span className="font-bold">{u.employeeId}</span>{" "}
                          • {u.email}
                        </p>
                    ...
                    <div className="md:col-span-2">
                      <span className="block font-medium text-slate-700 leading-snug">
                        {getDepartmentById(u.departmentId)?.name ||
                          "ไม่ระบุแผนก"}
                      </span>
                    ...
                    <div className="md:col-span-2 text-center md:text-left">
                      ...
                      currentUser.id !== u.id ? (
                        ...
                        <span
                          className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full font-bold font-mono text-[9.5px] uppercase ${
                            displayedRole === "Admin"
                              ? "bg-[#e51a24]/10 text-[#e51a24] border border-[#e51a24]/20"
                              : displayedRole === "Editor"
                                ? "bg-indigo-50 text-[#15329c] border border-indigo-150"
                        ...
                        >
                          {displayedRole === "Admin"
                            ...
                            : displayedRole === "Editor"
                              ...
                              : "Viewer"}
                          {currentUser.id === u.id && (
                            <span className="text-[7.5px] lowercase font-sans">
                              (คุณเอง)
                            </span>
                          )}
                        </span>
                    ...
                    <div className="md:col-span-1 text-center md:text-left">
                      ...
                      currentUser.id !== u.id ? (
                        ...
                        <span
                          className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full font-bold text-[9.5px] border ${
                            u.status === "Suspended"
                              ? "bg-amber-50 text-amber-600 border-amber-250"
                              : u.status === "Terminated"
                                ? "bg-rose-600/10 text-rose-600 border border-rose-200"
                      ...
                      {currentUser.role === "Admin" ? (
                        <>
                          {currentUser.id !== u.id ? (
                            ...
                            <span
                              className="text-[9.5px] text-slate-400 italic bg-slate-50 border border-slate-150 p-1 rounded cursor-not-allowed"
                              title="ความปลอดภัย ISO: สมาชิกไม่ได้รับอนุญาตให้ทำการแก้ไขสิทธิ์หรือรายละเอียดข้อมูลของท่านเองด้วยตัวท่านเองในระบบ"
                            >
                              ไม่ให้แก้ตนเอง
                            </span>
                          )}
                          <button
                            ...
                            onClick={() => {
                              if (currentUser.id === u.id) {
                                alert(
                                  "❌ คุณไม่สามารถลบบัญชีแอดมินหลักที่กำลังล็อกอินอยู่ได้ครับ",
                                );
                                return;
                              }
                              if (
                                window.confirm(
                                  `⚠️ คำเตือนระบบ ISO: คุณมีตัวเลือกเปลี่ยนสถานะเป็น [Suspended / Terminated] เพื่อระงับสิทธิ์ล็อกอินโดยคงประวัติไว้\n\nหากยังยืนยันจะกดลบ บัญชีและการประเมินสิทธิ์พนักงาน "${u.name}" จะถูกลบถาวรทันที! ต้องการลบหรือไม่?`,
                                )
                              ) {
                                onDeleteUser(u.id);
                              }
          ...
          <div className="lg:col-span-1 bg-white p-5 rounded-2xl border border-slate-200 space-y-4 text-left">
            ...
            <div className="space-y-3">
              ...
              {currentUser.role === "Admin" || currentUser.role === "Editor" ? (
                ...
                >
                  <span className="flex items-center gap-1.5">
                    ...
                    {currentUser.role === "Editor" && (
                      <span className="text-[7.5px] bg-emerald-100 text-emerald-800 font-bold px-1 rounded">
                        My Dept
                      </span>
                    )}
                  </span>
              ...
              >
                <span className="flex items-center gap-1.5">
                  ...
                  {currentUser.role === "Viewer" && (
                    <span className="text-[7.5px] bg-indigo-100 text-indigo-800 font-bold px-1 rounded">
                      My Profile
                    </span>
                  )}
                </span>
            ...
            {reportType === "DEPARTMENT" && (
              <div className="pt-3 border-t border-slate-150 space-y-1.5">
                ...
                <select
                  value={selectedRepDeptId}
                  onChange={(e) => setSelectedRepDeptId(e.target.value)}
                  disabled={currentUser.role === "Editor"}
                  className="w-full bg-white disabled:bg-slate-50 border border-slate-200 p-2 rounded-xl text-xs text-slate-700 focus:ring-1 focus:ring-indigo-500 cursor-pointer disabled:cursor-not-allowed"
                >
            ...
            <div className="pt-3 border-t border-slate-150 space-y-2 text-left">
              ...
              {currentUser.role === "Admin" || currentUser.role === "Editor" ? (
                ...
                >
                  ...
                  ส่งออก CSV: แผนก{" "}
                  {getDepartmentById(selectedRepDeptId)?.name || ""}
                </button>
              ) : (
                <div
          ...
          <div className="lg:col-span-3 bg-white rounded-2xl border border-slate-200 p-6 shadow-sm space-y-6">
            ...
            <div className="border-b border-[#e1ded5] pb-4 flex flex-col sm:flex-row justify-between items-start gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <span className="text-[9px] text-[#e51a24] font-bold uppercase tracking-wider block">
                    ROYAL MEIWA PAX CO., LTD. // 改善
                  </span>
                  <h3 className="font-extrabold text-sm sm:text-base text-slate-800 mt-0.5">
                    {reportType === "ALL" &&
                      "📊 รายงานสรุปผลสัมฤทธิ์ศึกษาการเรียนรู้และการสอบผ่าน (ภาพรวมพนักงานทั้งหมด)"}
                    {reportType === "DEPARTMENT" &&
                      `🏭 รายงานวิเคราะห์ความคืบหน้าระดับแผนกงาน: ${getDepartmentById(selectedRepDeptId)?.name || ""}`}
                    {reportType === "INDIVIDUAL" &&
                      `👤 ใบรายงานการเรียนรู้และวุฒิความรู้รายตัวพนักงาน: ${visibleUsers.find((u) => u.id === selectedRepUser)?.name}`}
                    {reportType === "SEARCH_LOGS" &&
                      "🔍 รายงานสืบค้นและวิเคราะห์ช่องว่างความรู้ (Search Queries & Knowledge Gaps Analysis)"}
                  </h3>
                </div>
              </div>
              ...
              <div className="space-y-4 font-sans text-xs">
                <div className="grid grid-cols-3 gap-4 text-center">
                  <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl">
                    ...
                    <strong className="text-xl text-slate-800 block mt-1">
                      {visibleUsers.length} คน
                    </strong>
                  </div>
                  <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl">
                    <span className="block text-slate-400 text-[10px] font-bold">
                      คอร์สฝึกอบรมรวมในระบบ
                    </span>
                    <strong className="text-xl text-[#15329c] block mt-1">
            ...
            {reportType === "DEPARTMENT" && (
              <div className="space-y-4 font-sans text-xs">
                {(() => {
                  const isMain =
                    getDepartmentById(selectedRepDeptId)?.parentId === null;
                  const deptUsers = isMain
                    ? visibleUsers.filter(
                        (u) =>
                          getMainDepartmentOf(u.departmentId)?.id ===
                          selectedRepDeptId,
                      )
                    : visibleUsers.filter(
                        (u) => u.departmentId === selectedRepDeptId,
                      );
                      ...
                      <div className="p-3 bg-indigo-50/30 border border-indigo-100 rounded-xl flex items-center justify-between">
                        <div>
                          <span className="text-[10px] font-bold text-indigo-800 uppercase block">
                            ตัววัดค่าแผนกงาน (Department Index)
                          </span>
                          <p className="text-xs text-slate-600 mt-0.5">
                            พบบุคลากรสังกัดแผนกหมุนเวียนนี้ทั้งหมดจำนวน:{" "}
                            <strong>{deptUsers.length} ท่าน</strong>
                          </p>
                        </div>
                        <span className="text-xs font-mono font-black text-indigo-700 bg-white border px-3 py-1.5 rounded-lg shadow-xs">
                          {getDepartmentById(selectedRepDeptId)?.name || ""} (
                          {getDepartmentById(selectedRepDeptId)?.code || ""})
                        </span>
                      </div>
                            ...
                            {deptUsers.length === 0 ? (
                              ...
                              deptUsers.map((u) => {
                                ...
                                return (
                                  ...
                                  >
                                    <td className="p-2.5 font-mono font-semibold text-slate-655">
                                      {u.employeeId}
                                    </td>
                                    <td className="p-2.5 font-bold text-slate-805">
                                      {u.name}
                                    </td>
            ...
            {reportType === "INDIVIDUAL" && (
              <div className="space-y-4 font-sans text-xs">
                {(() => {
                  const u = visibleUsers.find(
                    (userItem) => userItem.id === selectedRepUser,
                  );
                  if (!u)
                    return <p className="text-slate-400">โปรดเลือกพนักงาน</p>;
                  ...
                  const reqCourseIds = getRequiredCoursesForPosition(
                    u.position,
                  );
                  ...
                  return (
                    <div className="space-y-4">
                      ...
                      <div className="flex flex-col sm:flex-row gap-4 items-center bg-slate-50/50 p-4 rounded-xl border border-slate-200">
                        ...
                        <div className="min-w-0 flex-1 text-center sm:text-left space-y-1">
                          ...
                          <p className="text-xs text-slate-500 font-mono">
                            ...
                            | อีเมลติดต่อ: {u.email} | โทร. {u.phone}
                          </p>
                          <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2.5 pt-1">
                            <span className="bg-indigo-50 text-[#15329c] font-bold text-[10px] px-2.5 py-0.5 rounded border border-indigo-150">
                              ฝ่าย:{" "}
                              {getDepartmentById(u.departmentId)?.name || ""} (
                              {getDepartmentById(u.departmentId)?.code || ""})
                            </span>
                            <span className="bg-slate-100 text-slate-700 font-bold text-[10px] px-2.5 py-0.5 rounded border">
                              ตำแหน่ง: {u.position}
                            </span>
                          </div>
                        </div>
                      ...
                      <div className="space-y-2">
                        <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                          หลักสูตรและคะแนนสอบที่ประเมินบันทึก
                        </span>
                          ...
                          <table className="w-full text-left border-collapse">
                            <thead>
                              <tr className="bg-slate-50 font-bold border-b border-slate-200 p-2.5 text-[10px] text-slate-500 uppercase">
                                ...
                                <td className="p-2.5">
                                  ชื่อหลักสูตรอบรมความปลอดภัย/วิชาชีพ
                                </td>
                                <td className="p-2.5 text-center">
                                  คะแนนเกณฑ์สอบ
                                </td>
                            ...
                            <tbody className="divide-y divide-slate-150">
                              {reqCourseIds.map((courseId) => {
                                ...
                                const progressObj =
                                  userCompletedProgresses.find(
                                    (p) => p.courseId === courseId,
                                  );
                                const examObj = userExamLogs.find(
              ...
              <div className="space-y-6 font-sans text-xs">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 text-center">
                  <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl">
                    <span className="block text-slate-400 text-[10px] font-bold uppercase">
                      ยอดสะสมการสืบค้นข้อมูล
                    </span>
                    <strong className="text-xl text-slate-800 block mt-1">
                      {searchLogs.length} ครั้ง
                    </strong>
                  </div>
                  <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl">
                    <span className="block text-slate-400 text-[10px] font-bold uppercase">
                  ...
                  <div className="space-y-4">
                    <div className="p-3.5 bg-amber-50/50 border border-amber-200/80 rounded-xl leading-relaxed text-slate-750">
                      <p className="font-bold text-amber-900 flex items-center gap-1.5 text-xs">
                        ...
                        หลักฐานเชิงประจักษ์ของการแก้ไขปรับปรุงอย่างต่อเนื่อง
                        (ISO 9001 Clause 10 - Improvement & Clause 7.2)
                      </p>
                      <p className="text-[10.5px] text-slate-600 mt-1">
                        เมื่อผู้ปฏิบัติงานหน้างานค้นหาหัวข้อความปลอดภัยหรือแนวปฏิบัติ
                        แล้วระบบแสดงว่า{" "}
                        <span className="font-bold text-amber-700">
                          "ไม่พบผลลัพธ์ข้อมูล"
                        </span>{" "}
                        คำเหล่านั้นจะถูกแยกมาสรุปเป็นสัญลักษณ์ความเสี่ยงช่องว่างความรู้เพื่อตรวจพิจารณา
                        คุณสามารถบันทึกมอบหมาย SOP
                        เพื่อเป็นหลักฐานให้ผู้ตรวจประเบียบยอมรับได้ค่ะ
                      </p>
                    </div>
                        ...
                        <thead>
                          <tr className="bg-slate-50 font-bold border-b border-slate-200 p-2.5 text-[10px] text-slate-500 uppercase">
                            ...
                            <td className="p-3 text-center">
                              สถานะช่องว่างประเมิน
                            </td>
                            <td className="p-3 text-right">
                              ความคืบหน้าการอุดและ Resolution Action
                            </td>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-150">
                          {(() => {
                            const failedQueries = searchLogs.filter(
                              (log) => !log.hasResult,
                            );
                                    ...
                                    {isResolved ? (
                                      ...
                                      <div className="bg-slate-50 border border-slate-200 p-3 rounded-xl text-left space-y-2.5 max-w-sm ml-auto">
                                        <div className="space-y-1">
                                          <label className="text-[9.5px] font-bold text-slate-500 block">
                                            วิศวกรผู้ดูแลระบบ /
                                            ผู้เชี่ยวชาญรับมอบงาน:
                                          </label>
                                          <input
                                            type="text"
                                        ...
                                        <div className="space-y-1">
                                          <label className="text-[9.5px] font-bold text-slate-500 block">
                                            มาตรการแก้ไขและอ้างอิงเอกสารหลักสูตร
                                            (SOP Action):
                                          </label>
                                          <textarea
                ...
                {searchReportTab === "TOP_KEYWORDS" && (
                  <div className="space-y-4">
                    <p className="text-[11px] text-slate-500 leading-normal">
                      รายงานสถิติตัวแปรความสนใจของบุคลกร
                      ผ่านปริมาณสถิติการสืบค้นความถี่ปานกลาง
                      เพื่อใช้ทบทวนความพร้อมหัวข้อหลักสูตรการจัดตั้งห้องฝึกอบรมโรงเรียนวิชาชีพในโรงงาน
                      Meiwa PAX
                    </p>
                      ...
                      <table className="w-full text-left border-collapse">
                        <thead>
                          <tr className="bg-slate-50 font-bold border-b border-slate-200 p-2.5 text-[10px] text-slate-500 uppercase">
                            ...
                            <td className="p-3 text-center">
                              กลุ่มผู้สืบค้นหลักประเมิน
                            </td>
                            <td className="p-3 text-right">
                              สถานะคลังข้อมูลระเบียบปัจจุบัน
                            </td>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-150 font-sans text-left">
                          {(() => {
                            ...
                            searchLogs.forEach((log) => {
                              ...
                              if (
                                !groupedQueries[log.keyword].users.includes(
                                  log.userId,
                                )
                              ) {
                                groupedQueries[log.keyword].users.push(
                                  log.userId,
                                );
                              }
                            });

                            const sorted = Object.values(groupedQueries).sort(
                              (a, b) => b.count - a.count,
                            );
                            ...
                            return sorted.map((item, idx) => (
                              <tr
                                ...
                                className="hover:bg-slate-50/40 border-b border-slate-100"
                              >
                                <td className="p-3 font-semibold text-slate-800 text-[11.5px]">
                                  "{item.keyword}"
                                </td>
                                <td className="p-3 text-center font-mono font-black text-indigo-700 text-xs">
                                  {item.count} ครั้ง
                                </td>
                ...
                {searchReportTab === "RAW_LOGS" && (
                  <div className="space-y-4 font-sans text-left">
                    <p className="text-[11px] text-slate-500 leading-normal">
                      ประวัติการสแกนและดึงสืบค้นสดรายเซสชันจากผู้ใช้จริง
                      เป็นข้อมูลสารสนเทศดิบพับลิชเพื่อเก็บเป็นเอกสารตรวจสอบสอบบันทึกพนักงานตามหลักความสืบกลับได้
                      (Traceability audit)
                    </p>
                          ...
                          {searchLogs
                            ...
                            .map((log, idx) => {
                              ...
                              return (
                                ...
                                >
                                  <td className="p-3 font-mono text-[#15329c] font-black">
                                    {log.id}
                                  </td>
                                  ...
                                  <td className="p-3 text-right font-bold">
                                    {log.hasResult ? (
                                      ...
                                      <span
                                        className="text-amber-700 bg-amber-50 px-2 py-0.5 rounded border border-amber-100 font-bold text-[9.5px]"
                                        id={`tag-gap-${log.id}`}
                                      >
                                        GAP DETECTED
                                      </span>
        ...
        (currentUser.role === "Viewer" ? (
          <div className="bg-white p-12 rounded-2xl border border-slate-200 shadow-sm text-center max-w-lg mx-auto my-12 space-y-4">
            ...
            <p className="text-xs text-slate-500 leading-relaxed">
              ขออภัย บัญชีของคุณมีบทบาทเป็น{" "}
              <span className="font-bold text-amber-600">
                ผู้เข้าชมทั่วไป (Viewer)
              </span>{" "}
              จึงไม่ได้รับสิทธิ์เข้าถึงหรือปรับแก้ข้อมูลใดๆ
              ในส่วนของฐานข้อมูลพนักงานกลางรอลงทะเบียนนี้
            </p>
            <p className="text-[10px] text-slate-400">
              *หากต้องการสิทธิ์ในการจัดการ
              กรุณาติดต่อผู้ดูแลระบบเพื่อทำการอัปเกรดบทบาทเป็น Editor หรือ Admin
            </p>
            <button
              onClick={() => setActiveTab("MEMBERS")}
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-lg font-black text-slate-800">
                      ระบบนำเข้าฐานข้อมูลพนักงานความปลอดภัยโรงงาน (Employee
                      Master Onboarding Database)
                    </h3>
                    <p className="text-xs text-slate-500 mt-0.5">
                      ผู้อนุมัติ (Admin หรือ Editor)
                      สามารถนำเข้าบัญชีล่วงหน้าด้วยไฟล์เพื่อลดความเสี่ยงจากการขโมยตัวสิทธิ์
                      โดยผู้ปฏิบัติงานใหม่จะป้อนเฉพาะรหัสพนักงานในการสมัครบัญชี
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
                ...
                <div className="p-5 bg-slate-50 rounded-2xl border border-dashed border-slate-300 hover:border-[#15329c] transition-colors group relative flex flex-col justify-between space-y-4">
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500 leading-normal">
                      ลากและวางไฟล์ใบรายชื่อพนักงาน
                      แผนกรวมระดับพนักงานที่เป็นไฟล์ Excel (<code>.xlsx</code>,{" "}
                      <code>.csv</code>) หรือรายงาน PDF จากแผนก HR ที่นี่
                      ระบบจะประมวลผลการจัดโครงสร้าง คาดคะเนแบบฟอร์มอัตโนมัติ
                    </p>
                    ...
                    <label className="mt-4 border border-dashed border-slate-300 rounded-xl p-6 bg-white hover:bg-slate-100/50 cursor-pointer flex flex-col items-center justify-center text-center transition group-hover:scale-[0.99] block">
                      ...
                      <span className="text-xs font-bold text-slate-700 block">
                        คลิก หรือลากไฟล์ Excel / PDF มาวางที่นี่
                      </span>
                      <span className="text-[10px] text-slate-400 block mt-1 font-mono">
                        ขนาดยอมรับสูงสุด 25MB • สนับสนุน XLSX, XLS, PDF, CSV,
                        IMAGE
                      </span>
                      <input
                        type="file"
                ...
                <div className="p-5 bg-slate-50 rounded-2xl border border-[#e1ded5] flex flex-col justify-between space-y-3">
                  <div className="space-y-1.5 flex-1">
                    ...
                    <p className="text-[11px] text-slate-500 leading-normal">
                      คัดลอกตารางข้อมูลพนักงานจาก Google Sheets หรือไฟล์ชีตใดๆ
                      แล้ววางลงในกล่องข้อความด้านล่างโดยตรง (ระบุข้อมูลพนักงาน
                      คั่นด้วยเครื่องหมายจุลภาค <code>,</code> หรือเว้นวรรค
                      Tab):
                    </p>
                    <span className="block text-[9.5px] font-mono text-slate-655 bg-white border border-slate-200 p-1.5 rounded leading-normal">
                      ...
                      <strong className="text-indigo-700">
                        รหัสพนักงาน , ชื่อนามสกุล , แผนก , ตำแหน่ง , ระดับ ,
                        วันเริ่มงาน , อีเมล , เบอร์โทร
                      </strong>
                    </span>
                  </div>
                    ...
                    <button
                      ...
                      onClick={() => {
                        const el = document.getElementById(
                          "tabular-paste-textarea",
                        ) as HTMLTextAreaElement;
                        if (!el || !el.value.trim()) {
                          alert(
                            "กรุณากรอกข้อมูลพนักงานเพื่อจัดประมวลวิเคราะห์ข้อมูลประสาน",
                          );
                          return;
                        }
                        ...
                        if (newlyImported.length > 0) {
                          const duplicatesMerged = newlyImported.filter(
                            (emp) =>
                              !employeeMaster.some(
                          ...
                          if (duplicatesMerged.length > 0) {
                            ...
                            alert(
                              `⚠️ ข้อมูลพนักงานทั้ง ${parsedCount} ท่านจากชีตนี้เคยถูกนำเข้าระบบไว้ก่อนหน้านี้แล้วค่ะ`,
                            );
                          }
                          el.value = ""; // clear text area
                        } else {
                          alert(
                            "❌ ไม่พบโครงสร้างข้อมูลตามแถว! กรุณาตรวจสอบการคั่นด้วยเครื่องหมายจุลภาค (,) หรือแท็บ",
                          );
                        }
                      }}
              ...
              <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4">
                <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                  <h4 className="font-extrabold text-sm text-slate-800 flex items-center gap-2">
                    ...
                    ผลการสแกนและสกัดข้อมูลด้วยระบบประมวลผลอัตโนมัติ (Parser
                    Engine Results)
                  </h4>
                  {parsedEmployees.length > 0 && (
                    <button
                ...
                {parserLoading && (
                  <div className="flex flex-col items-center justify-center py-6 space-y-3">
                    ...
                    <p className="text-xs font-semibold text-slate-600">
                      กำลังประมวลผลไฟล์และจำแนกข้อมูลคอลัมน์ โปรดรอสักครู่...
                    </p>
                    <p className="text-[10px] text-slate-400 font-mono">
                      ระบบใช้คลังข้อมูลชีตและ Express AI OCR Extraction
                      ในการวิเคราะห์รูปแบบ
                    </p>
                  </div>
                )}
                ...
                {parsedEmployees.length > 0 && showImportPreview && (
                  <div className="space-y-4">
                    <p className="text-xs text-slate-500 leading-relaxed">
                      ตรวจสอบรายการด้านล่างเพื่อตรวจสอบความถูกต้องของฟิลด์ก่อนบันทึกเข้าสู่ฐานข้อมูลพนักงานความปลอดภัยโรงงานหลัก
                      (Employee Master DB):
                    </p>
                            ...
                            return (
                              ...
                              >
                                <td className="p-3 font-mono font-bold text-[#15329c]">
                                  {emp.employeeId}
                                  {isDuplicate && (
                                    <span className="ml-1 text-[9px] bg-amber-100 text-amber-800 px-1.5 py-0.5 rounded font-sans font-bold">
                                      ซ้ำในระบบ
                                    </span>
                                  )}
                                </td>
                    ...
                    <div className="flex justify-end gap-3 pt-2">
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          const duplicatesMerged = parsedEmployees.filter(
                            (emp) =>
                              !employeeMaster.some(
                                (exist) => exist.employeeId === emp.employeeId,
                              ),
                          ...
                          if (duplicatesMerged.length > 0) {
                            ...
                            alert(
                              `🎉 สำเร็จ! บันทึกนำเข้าข้อมูลพนักงานจำนวน ${duplicatesMerged.length} รายการเข้าสู่ระบบเรียบร้อยแล้วค่ะ`,
                            );
                          } else {
                            alert(
                              `⚠️ ไม่มีพนักงานรายใหม่ ถูกข้ามทั้งหมดเนื่องจากรหัสพนักงานซ้ำซ้อนกับระบบฐานข้อมูลที่มีอยู่แล้ว`,
                            );
                          }
                          setParsedEmployees([]);
                          setParserSuccessMsg(null);
                      ...
                      >
                        บันทึกฐานรายชื่อพนักงาน ({parsedEmployees.length}{" "}
                        รายการ)
                      </button>
                    </div>
                  </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden space-y-4 p-5 pb-3">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div className="space-y-0.5">
                  <span className="text-[10px] text-[#e51a24] font-bold uppercase tracking-wider block font-mono">
                    Official HR registry for onboarding automation
                  </span>
                  <h4 className="font-extrabold text-sm text-slate-800">
                    รายชื่อระเบียนพนักงานกลางรอนำเข้าเปิดสิทธิ์ใช้งานระบบ (
                    {employeeMaster.length} รายชื่อทั้งหมด)
                  </h4>
                </div>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    onClick={() => {
                      if (
                        window.confirm(
                          "คุณแน่ใจหรือไม่ว่าต้องการเพิกถอนข้อมูลพนักงานที่ลงทะเบียนรอสมัครออกทั้งหมด?",
                        )
                      ) {
                        onUpdateEmployeeMaster([]);
                      }
                  ...
                  <tbody className="divide-y divide-slate-100 text-[11px]">
                    {employeeMaster.length === 0 ? (
                      <tr>
                        ...
                        >
                          🚫 ไม่มีระเบียนข้อมูลพนักงานรอนำสมัคร กรุณาลากวางไฟล์
                          Excel/PDF
                          หรือคัดลอกส่วนข้อมูลชีตด้านบนเพื่อนำเข้ารายชื่อใหม่ได้ทันทีค่ะ
                        </td>
                      </tr>
                    ) : (
                      employeeMaster.map((emp, idx) => {
                        const isLinkedUser = users.some(
                          (u) =>
                        ...
                        return (
                          ...
                          >
                            <td className="p-3 font-mono font-extrabold text-[#15329c]">
                              {emp.employeeId}
                            </td>
        ...
        <div className="fixed inset-0 bg-slate-900/60 overflow-y-auto flex items-start sm:items-center justify-center p-4 sm:p-6 md:p-10 z-50 animate-fade-in">
          <div className="bg-white rounded-3xl border border-slate-200 shadow-xl max-w-md w-full overflow-hidden flex flex-col my-auto">
            <div className="bg-[#15329c] text-white p-4.5 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-extrabold text-sm">
                  เพิ่มบัญชีสมาชิกและกำหนดสิทธิ์ RMP
                </h3>
              </div>
              <button
            ...
            <form onSubmit={handleCreateUserSubmit} className="p-6 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      className="w-full bg-white border border-slate-200 p-2 rounded-lg text-xs focus:ring-1 focus:ring-[#15329c]"
                    />
                    {getPositionsForDepartment(newUser.departmentId).length >
                      0 && (
                      <p className="text-[9.5px] text-slate-400 mt-1">
                        ตัวอย่างตำแหน่งที่แนะนำ:{" "}
                        {getPositionsForDepartment(newUser.departmentId)
                          .slice(0, 4)
                          .join(", ")}
                        {getPositionsForDepartment(newUser.departmentId)
                          .length > 4
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-500 block">
                  ค่าเริ่มต้นบทบาทสิทธิ์ป้อนระบบ (Default Role):
                </label>
                <select
                  value={newUser.role}
                  onChange={(e) =>
              ...
              <div className="space-y-1.5 p-3.5 bg-slate-50 border border-slate-200/80 rounded-2xl">
                <span className="text-[10px] font-extrabold text-slate-500 block uppercase">
                  รูปภาพโปรไฟล์จริง (Profile Photo):
                </span>
        ...
        <div className="space-y-6 animate-fade-in">
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-black text-slate-800">
                    บันทึกประวัติการกระทำของระบบแบบถาวร (System Compliance Audit
                    Logs)
                  </h3>
                  <p className="text-xs text-slate-500 mt-0.5">
                    บันทึกประวัติกิจกรรม การอนุมัติ แก้ไขสิทธิ์
                    และบริหารงานระบบผู้ใช้งานตามข้อกำหนด ISO 9001 และ ISO 14001
                  </p>
                </div>
              </div>
              <button
                ...
                onClick={() => {
                  ...
                  const csvHeaders = [
                    "ลำดับกิจกรรม (UID)",
                    "ประเภทคำสั่ง (Action)",
                    "วิชาชีพ/หัวข้อ (Details)",
                    "ผู้ปฏิบัติการ (Performed By)",
                  ...
                  link.setAttribute(
                    "download",
                    `RMP_Factory_System_Audit_Logs_${new Date().toISOString().split("T")[0]}.csv`,
                  );
                  document.body.appendChild(link);
                  link.click();
                  document.body.removeChild(link);
                  alert(
                    "🎉 ส่งออกไฟล์บันทึกประวัติแอดมินกลาง (ISO Clause 9.3/10.2 Compliant) เรียบร้อย!",
                  );
                }}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-2">
              ...
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                ...
                <span className="text-sm font-extrabold text-emerald-600 font-mono mt-1 block">
                  {
                    systemAuditLogs.filter((l) => l.action === "APPROVE_MEMBER")
                      .length
                  }{" "}
                  บัญชี
                </span>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-4">
            <h4 className="text-xs font-bold text-slate-700 uppercase">
              🔍 ตัวกรองประวัติการเข้าใช้งานแอดมิน
            </h4>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              >
                ...
                <option value="APPROVE_MEMBER">
                  🟢 อนุมัติพนักงานใหม่ (APPROVE)
                </option>
                <option value="UPDATE_MEMBER">
                  🔵 แก้ไขข้อมูลบันทึก (UPDATE_MEMBER)
                </option>
              ...
              {filteredAuditLogs.length === 0 ? (
                <div className="text-center py-12 border border-dashed border-slate-200 rounded-2xl bg-slate-50/50">
                  <div className="text-slate-400 text-xs font-medium">
                    ไม่พบข้อมูลบันทึกตามค้นหาตัวกรองนี้
                  </div>
                </div>
              ) : (
                <div className="divide-y divide-slate-100 max-h-[500px] overflow-y-auto pr-1 space-y-2.5">
                  {filteredAuditLogs.map((log) => {
                    ...
                    if (log.action === "APPROVE_MEMBER") {
                      ...
                      actionBadgeColor =
                        "bg-[#15329c]/10 text-[#15329c] border-indigo-200";
                    ...
                    return (
                      <div
                        key={log.id}
                        className="p-4 rounded-xl border border-slate-150 hover:bg-slate-50/50 transition flex flex-col md:flex-row md:items-center justify-between gap-3 bg-white text-xs"
                      >
                        <div className="space-y-1.5">
                          <div className="flex items-center gap-2 flex-wrap">
                            <span
                              className={`px-2 py-0.5 rounded text-[10px] font-bold border ${actionBadgeColor}`}
                            >
                              {actionName}
                            </span>
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-xl max-w-md w-full overflow-hidden flex flex-col my-auto">
            <div className="bg-[#15329c] text-white p-4.5 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-extrabold text-sm text-[13px]">
                  📝 แก้ไขรายละเอียดและสิทธิ์สมาชิก
                </h3>
              </div>
              <button
                type="button"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <p className="text-[8.5px] text-slate-400">
                    *เชื่อมโยงกับรหัสผ่านระบบ
                  </p>
                </div>

                <div className="space-y-1">
                  <label className="text-[10px] font-extrabold text-slate-500 block uppercase">
                    รหัสผ่านเข้าใช้ (PIN 6 หลัก):
                  </label>
                  <input
                    type="text"
                    maxLength={6}
                  ...
                  />
                  <p className="text-[8.5px] text-amber-600">
                    *ระบุเฉพาะตัวเลข 6 หลักจริง
                  </p>
                </div>
              </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      placeholder={
                        getPositionsForDepartment(editForm.departmentId)[0]
                          ? `เช่น "${getPositionsForDepartment(editForm.departmentId)[0]}"`
                          : "เช่น เจ้าหน้าที่..."
                      }
                      required
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  >
                    ...
                    <option value="Admin">
                      Admin (ผู้ดูแลระบบหลักควบคุมสิทธิ์)
                    </option>
                  </select>
                </div>
