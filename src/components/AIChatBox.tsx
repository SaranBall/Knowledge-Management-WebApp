...
import { 
  Send, Bot, User as UserIcon, Database, ArrowRight, Sparkles, 
  HelpCircle, CheckCircle, AlertTriangle, FileText, Plus, Trash2, 
  Eye, BookOpen, Key, Link as LinkIcon, RefreshCw, Layers
} from 'lucide-react';
import { User, DocumentItem, KBArticle, Course, CustomResource } from '../types';
...
export const AIChatBox: React.FC<AIChatBoxProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'CHAT' | 'RESOURCES' | 'INFOGRAPHIC'>('INFOGRAPHIC');
  const [chatMode, setChatMode] = useState<'CONVERSATIONAL' | 'SIMPLE_QA'>('CONVERSATIONAL');
  ...
  const [resourceType, setResourceType] = useState<'Text' | 'URL' | 'Manual'>('Manual');

  // Preview State
  const [previewCitation, setPreviewCitation] = useState<{ title: string; type: string; content: string } | null>(null);
  ...
  const processRAGQuery = (query: string): { responseText: string; citations: any[] } => {
    const q = query.toLowerCase().trim();

    if (q.includes("vacuum") || q.includes("วิธีตรวจสูญญากาศ") || q.includes("ตรวจวัดสูญญากาศ")) {
      return {
    ...
    const calculateScore = (sourceText: string, searchPhrase: string): number => {
      let score = 0;
    ...
    if (bestMatch.type.includes('เอกสารระบบ')) {
      ...
      const problem = docCleanContent.split('ปัญหาชำรุด: ')[1]?.split('\n\nวิเคราะห์สาเหตุ')[0] || '';
      const cause = docCleanContent.split('วิเคราะห์สาเหตุ (Root Cause): ')[1]?.split('\n\nขั้นตอนการแก้ไข')[0] || '';
      const solution = docCleanContent.split('ขั้นตอนการแก้ไขโดยผู้เชี่ยวชาญ (Solution):\n')[1]?.split('\n\nมาตรการป้องกัน')[0] || '';
      const prevention = docCleanContent.split('มาตรการป้องกันระยะยาว (Prevention):\n')[1]?.split('\n\nเอกสารเชื่อมโยง')[0] || '';

      synthesis += `*   **1. อาการของปัญหา (Problem):** ${problem}\n`;
      synthesis += `*   **2. สาเหตุหัวใจของปัญหา (Root Cause):** <span className="text-[#a21217] font-bold">${cause}</span>\n\n`;
      ...
      const lessonContent = bestMatch.content.split('เนื้อหาบทเรียนย่อย')[1] || bestMatch.content;
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    try {
        ...
        const botMessage: ChatMessage = {
          ...
          text: `### 🔍 ผลลัพธ์การค้นหาแบบ Q&A (Direct Search Snippets)\n\nระบบตรวจพบเอกสารที่ระบุคำค้นหาใกล้เคียงที่สุดดังนี้ครับ:\n\n${
            ragResult.citations.length > 0 
              ? ragResult.citations.map((c, i) => `**อันดับที่ ${i+1}.** [${c.type}] **"${c.title}"** \n*ตัวอย่างเนื้อหา:* \n${c.content.slice(0, 200)}...\n\n`).join('')
      ...
      console.warn("Express Semantic RAG failed, falling back to local client keyword search:", err);
  ...
  const handleCreateResourceSubmit = (e: React.FormEvent) => {
    ...
    setResourceTitle('');
    setResourceContent('');
    alert('🎉 อัปโหลดเอกสาร / ดึงพจนานุกรมความรู้สำหรับระบบ RAG เข้าคลังเรียบร้อยแล้ว!');
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-4 rounded-2xl border border-slate-200">
        <div className="flex flex-wrap items-center gap-1.5 bg-slate-100 p-1 rounded-xl">
          ...
          >
            ...
            ฐานข้อมูลสารสนเทศที่ระบบ Ingest {currentUser.role === 'Admin' && <span className="bg-emerald-500 text-white text-[8px] font-bold px-1 rounded">Admin</span>}
          </button>
        </div>

        <div className="text-slate-400 font-mono text-[10px] bg-slate-50 px-2.5 py-1 rounded">
          🧠 สิทธิใช้งาน: <strong className="text-slate-800 uppercase font-bold">{currentUser.role} PRIVILEGED</strong>
        </div>
      </div>
          ...
          <div className="lg:col-span-1 space-y-4 h-full flex flex-col justify-between">
            <div className="bg-white p-4 rounded-2xl border border-slate-200 space-y-4">
              <div className="flex items-center gap-1.5 pb-2 border-b border-slate-100">
                ...
                <h4 className="font-extrabold text-slate-800 text-xs">เครื่องมือค้นความรู้ RAG</h4>
              </div>

              {/* Bot selection options */}
              <div className="space-y-2">
                <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest">โหมดการค้นหาคำตอบ</span>
            ...
            <div className="bg-[#15329c]/5 p-4 rounded-2xl border border-[#15329c]/10 text-[10.5px] leading-relaxed text-slate-600">
              ...
              <p>ระบบใช้แนวทางสะพานเชื่อมโยงความรู้ (Semantic Mapping) แก้ปัญหาพิมพ์ศัพท์ต่างกัน รวมถึงป้องกัน <strong>AI Slop / Hallucination</strong> ห้ามเสนอแนะอุณหภูมิภายนอกที่อาจทำให้เกิดความร้อนเกินหรือไฟไหม้เครื่องจักร</p>
            </div>
          </div>
          ...
          <div className="lg:col-span-3 bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col h-full overflow-hidden">
            ...
            <div className="bg-[#15329c] text-white px-5 py-3.5 flex items-center justify-between border-b border-[#15329c]">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="font-extrabold text-xs tracking-tight">RMP AI Knowledge Assistant</h3>
                  <p className="text-[9px] text-slate-200 mt-0.5 font-mono">
                    ฐานข้อมูลดึงความรู้: {documents.length} เอกสารมาตรฐาน | {kbArticles.length} Kaizens | {courses.length} คอร์สอบรม
                  </p>
                </div>
              </div>
            ...
            <div className="flex-1 p-5 overflow-y-auto space-y-4 bg-slate-50/50">
              {messages.map((msg) => (
                ...
                >
                  <div className={`p-2 rounded-xl shrink-0 h-8 w-8 flex items-center justify-center text-white ${
                  ...
                  <div className="space-y-2">
                    ...
                    {msg.citations && msg.citations.length > 0 && (
                      <div className="space-y-1.5 p-1">
                        <span className="block text-[9.5px] font-extrabold text-slate-400 uppercase tracking-widest">
                          📄 แหล่งอ้างอิงยืนยันความถูกต้อง (Local Ingested Source)
                        </span>
                        <div className="flex flex-wrap gap-2">
                          {msg.citations.map((cite, cIdx) => (
                            ...
                            >
                              ...
                              <span>{cite.title.split(': ')[1]?.slice(0, 30) || cite.title}</span>
                    ...
                    <span className={`block text-[9px] text-slate-400 font-mono mt-1 ${msg.sender === 'user' ? 'text-right' : 'text-left'}`}>
                      {msg.timestamp}
                    </span>
                  </div>
                </div>
      ...
      {activeTab === 'RESOURCES' && (
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-slate-100">
            <div>
              <h3 className="font-extrabold text-slate-900 text-sm">ข้อมูลสารสนเทศทั้งหมดที่ระบบ AI สลักเชื่อมโยง</h3>
              <p className="text-xs text-slate-500">ข้อมูลสารสนเทศในองค์กรทั้งหมดที่ดึงมาประมวลผลเป็น RAG Ingestion ในขณะนี้</p>
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-center">
            <div className="p-4 bg-indigo-50 border border-indigo-100 rounded-2xl">
              <span className="block text-slate-500 text-[10px] font-bold uppercase">เอกสาร QP / WI ฉบับพับลิช</span>
              <strong className="text-2xl font-black text-indigo-900 block mt-1">{documents.filter(d => d.status === 'Published').length} ฉบับ</strong>
            </div>
            <div className="p-4 bg-emerald-50 border border-emerald-100 rounded-2xl">
              <span className="block text-slate-500 text-[10px] font-bold uppercase">ข้อยืดหยุ่น Kaizen & บทความเชิงช่าง</span>
              <strong className="text-2xl font-black text-emerald-900 block mt-1">{kbArticles.filter(k => k.status === 'Approved').length} บทความ</strong>
            </div>
            <div className="p-4 bg-amber-50 border border-amber-100 rounded-2xl">
              <span className="block text-slate-500 text-[10px] font-bold uppercase">บทเรียนย่อยในอคาเดมีออนไลน์</span>
            ...
            <div className="p-4 bg-rose-50 border border-rose-100 rounded-2xl">
              <span className="block text-slate-500 text-[10px] font-bold uppercase">เอกสาร / ความรู้เพิ่มเติม (Custom Ingestion)</span>
              <strong className="text-2xl font-black text-rose-900 block mt-1">{customResources.length} หัวข้อ</strong>
            </div>
          </div>
          ...
          {currentUser.role === 'Admin' ? (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 pt-2">
              ...
              <div className="lg:col-span-1 bg-[#15329c]/5 p-4 rounded-2xl border border-[#15329c]/10 space-y-4">
                ...
                <form onSubmit={handleCreateResourceSubmit} className="space-y-4">
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-slate-500 block">ชื่อคู่มือความรู้ / หัวข้ออ้างอิง:</label>
                    <input
                      type="text"
                      value={resourceTitle}
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-slate-500 block">ประเภทแหล่งที่มา:</label>
                    <select
                      value={resourceType}
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-slate-500 block">เนื้อหาความรู้อ้างอิง (สำหรับดึง RAG):</label>
                    <textarea
              ...
              <div className="lg:col-span-2 space-y-3">
                <span className="block text-xs font-bold text-slate-600">รายการข้อมูลเพิ่มเติมที่ป้อนเข้าระบบแล้ว</span>
                
                {customResources.length === 0 ? (
                  <div className="bg-slate-50 p-12 rounded-xl text-center border border-dashed border-slate-200 text-slate-400 text-xs">
                    ปัจจุบันยังไม่มีการป้อนข้อมูลเพิ่มเติมใดๆ คู่มือทั้งหมดอ้างอิงตรงจากเอกสารสารสนเทศมาตรฐาน
                  </div>
                ) : (
                  <div className="space-y-3 max-h-[350px] overflow-y-auto">
                    {customResources.map((res) => (
                      <div key={res.id} className="p-3.5 bg-white border border-slate-200 rounded-xl flex items-start justify-between gap-3 text-xs shadow-xs">
                        <div>
                          ...
                          <p className="text-slate-400 text-[9.5px] mt-1 font-mono">เพิ่มโดย: {res.addedBy} | {new Date(res.createdAt).toLocaleDateString()}</p>
                          <p className="text-slate-600 mt-2 line-clamp-3 bg-slate-50/50 p-2 rounded border border-slate-100">{res.content}</p>
                        </div>
            ...
            <div className="p-6 bg-slate-50 text-center text-slate-500 rounded-2xl border border-slate-150 text-xs space-y-2">
              ...
              <h5 className="font-bold text-slate-700">สิทธิ์ Viewer/Editor ของคุณไม่สามารถแก้ไขฐานสารสนเทศได้</h5>
              <p>เฉพาะแอดมินฝ่ายความรู้ คณะกรรมการกลาง เท่านั้นที่สามารถนำเข้าคู่มือ Manual ฉบับพิเศษเข้าระบบได้ หากมีคำแนะนำ Kaizen เพิ่มเติมรบกวนเสนอไปยังหัวหน้าฝ่ายของคุณครับ</p>
            </div>
          )}
        </div>
      )}
        ...
        <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200 space-y-8 animate-fade-in text-slate-800">
          ...
          <div className="text-center bg-gradient-to-r from-blue-700 via-[#15329c] to-indigo-800 text-white py-8 px-6 rounded-3xl shadow-md space-y-3 relative overflow-hidden">
            ...
            <h2 className="text-2xl md:text-4xl font-extrabold tracking-tight">
              เลิกเสียเวลา <span className="text-amber-300">"หาข้อมูล"</span> ให้ AI ช่วยหาคำตอบแทนคุณ
            </h2>
            <p className="text-xs md:text-sm text-slate-100 max-w-xl mx-auto font-medium leading-relaxed">
              สืบค้นและตอบคำถามอย่างแม่นยำ 24 ชั่วโมงจากคู่มือมาตรฐาน SOP, WI และองค์ความรู้เชิงช่าง Kaizen ของ บริษัท รอแยล เมอิวะ แพ็คซ์ จำกัด
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-rose-50/60 border border-rose-100 p-6 rounded-2xl space-y-4">
              ...
              <div className="space-y-3">
                {[
                  { title: "เอกสารกระจัดกระจาย หาไม่เจอ", desc: "คู่มือกระจัดกระจายอยู่ตามเครื่องคอมพิวเตอร์ ตู้เอกสาร หรือไฟล์แชร์แผนกต่าง ๆ" },
                  { title: "ใช้เวลาค้นหาหลายชั่วโมง", desc: "เสียเวลาทำงานหน้าเครื่องจักรและหน้าไลน์ผลิตเพื่อไปรื้อดูคู่มือมาแก้ปัญหาเฉพาะหน้า" },
                  { title: "เอกสารผิด Revision", desc: "เผลอใช้เอกสารฉบับเก่าที่ยกเลิกไปแล้วมาทำมาตรฐาน ทำให้สินค้าหลุดเกณฑ์คุณภาพ" },
                  { title: "ข้อมูลไม่เป็นมาตรฐาน", desc: "พนักงานทำงานตามใจตัวเอง ขาดเอกสารยึดเหนี่ยวมาตรฐานและขั้นตอนที่ถูกต้องแม่นยำ" },
                  { title: "ความรู้หายเมื่อพนักงานลาออก", desc: "องค์ความรู้เชิงลึก เทคนิคการแก้ปัญหาหน้าเครื่องติดตัวไปกับช่างที่ย้ายแผนกหรือลาออก" }
                ].map((item, index) => (
                  <div key={index} className="flex gap-3 bg-white p-3 rounded-xl border border-rose-100/50 hover:shadow-xs transition">
                    <span className="text-rose-550 shrink-0 text-sm font-bold">✕</span>
                    <div>
                      <strong className="text-slate-800 text-xs block">{item.title}</strong>
                      <span className="text-[10.5px] text-slate-500 block mt-0.5">{item.desc}</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-emerald-50/60 border border-emerald-100 p-6 rounded-2xl space-y-4">
              <h3 className="text-emerald-800 font-extrabold text-sm flex items-center gap-2">
                <span className="w-6 h-6 rounded-full bg-emerald-100 text-emerald-800 flex items-center justify-center font-bold">✓</span>
                AI Knowledge Base ช่วยโรงงานได้อย่างไร?
              </h3>
              <div className="space-y-3">
                {[
                  { title: "รวมทุกเอกสารไว้ในที่เดียว", desc: "รวบรวมไฟล์ SOP, Work Instruction (WI) และแบบฟอร์มเข้าสู่ฐานความรู้กลางของบริษัท" },
                  { title: "ค้นหาและตอบคำถามทันที", desc: "ประมวลผลคำตอบแบบ Semantic RAG รวดเร็วในระดับวินาที ไม่ต้องคลิกเปิดหาทีละหน้า" },
                  { title: "อ้างอิงแหล่งที่มาชัดเจน", desc: "ทุกคำตอบแสดง Citation อ้างอิงถึงชื่อและเวอร์ชันไฟล์อนุมัติจริงที่เปิดใช้งานอยู่" },
                  { title: "ข้อมูลเป็นมาตรฐานเดียวกัน", desc: "ทุกคนอ้างอิงคำตอบผ่านบอทฉบับล่าสุด (Latest Approved Only) ป้องกันปัญหาทำผิดเกณฑ์" },
                  { title: "ความรู้ไม่หาย แม้คนเปลี่ยน", desc: "บันทึกเทคนิคหน้าไลน์ วิธีถอดซ่อม และสูตรแก้ปัญหาในรูปฐานข้อมูลคลังสมองอัจฉริยะ" }
                ].map((item, index) => (
                  <div key={index} className="flex gap-3 bg-white p-3 rounded-xl border border-emerald-100/50 hover:shadow-xs transition">
          ...
          <div className="space-y-4">
            <div className="text-center space-y-1">
              <h3 className="text-xs font-black text-[#15329c] uppercase tracking-wider">
                10 USE CASES ที่ช่วยงานโรงงานได้จริง (คลิกหัวข้อเพื่อสั่งพิมพ์คำถามลงในแชททันที)
              </h3>
              <p className="text-xs text-slate-500">
                คุณสามารถทดลองใช้งาน AI Knowledge Base ตามกรณีการใช้งานยอดฮิตด้านล่างนี้ได้โดยการคลิกที่กล่องระบบ
              </p>
            </div>

            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              {[
                { id: 1, title: "1. ค้นหา SOP และ WI", icon: "🔍", query: "ขอขั้นตอนควบคุมกระบวนการเป่าฟิล์มและลามิเนตมาตรฐาน QP-PRD-001" },
                { id: 2, title: "2. ตอบคำถามด้านคุณภาพ", icon: "🛡️", query: "วิธีตรวจ Vacuum ทำอย่างไร?" },
                { id: 3, title: "3. ค้นหาข้อกำหนดลูกค้า", icon: "👥", query: "ขอกฎระเบียบข้อกำหนดด้านคุณภาพสินค้าเมื่อจัดส่งให้ลูกค้ากลุ่มบริโภค" },
                { id: 4, title: "4. ตรวจสอบ Specification", icon: "📄", query: "มาตรฐานตรวจสอบความชื้นคุณภาพเม็ดพลาสติก Moisture Tester ค่าควบคุมคือเท่าไหร่" },
                { id: 5, title: "5. ค้นหา Audit Finding", icon: "📝", query: "รายการสิ่งบกพร่องจากการตรวจประเมินภายใน มีประเด็นสำคัญเรื่องใดบ่อยบ้าง" },
                { id: 6, title: "6. ช่วยฝึกอบรมพนักงานใหม่", icon: "🎓", query: "บทเรียนเตรียมความพร้อมพนักงานคลังสินค้าใหม่ มีหัวข้อใดหลักๆ บ้าง" },
                { id: 7, title: "7. สรุปเอกสารยาวๆ", icon: "📚", query: "ช่วยสรุปกระบวนการจัดซื้อวัตถุดิบและพลาสติกคอมพาวด์ QP-PUR-001 ให้สั้นลงหน่อย" },
                { id: 8, title: "8. ค้นหา Root Cause", icon: "🔥", query: "สาเหตุการเกิดปัญหาฟองพลาสติกพรูหรือสิ่งแปลกปลอมในฟิล์มเป่า เกิดจากอะไร" },
                { id: 9, title: "9. ค้นหา CAPA", icon: "⚙️", query: "มาตรการปรับปรุงแก้ไข CAPA เรื่องอุณหภูมิกระบอกสูบสูงเกินไปที่ผ่านมาดำเนินการอย่างไร" },
                { id: 10, title: "10. Chatbot องค์กร 24 ชั่วโมง", icon: "💬", query: "สอบถามกฎระเบียบมาตรฐานความมั่นคงและเวลาเปิด-ปิดการใช้งานคลังสินค้า" }
              ].map((uc) => (
                <button
                  type="button"
                  key={uc.id}
                ...
                >
                  <span className="text-2xl block group-hover:scale-110 transition-transform">{uc.icon}</span>
                  <span className="font-extrabold text-[11px] text-slate-800 block line-clamp-2 leading-tight">{uc.title}</span>
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-200 space-y-4">
            <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest text-center">
              ผลลัพธ์เชิงธุรกิจที่องค์กร รอแยล เมอิวะ แพ็คซ์ จำกัด ได้รับ (RMP Strategic Outcomes)
            </h4>
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4 divide-y md:divide-y-0 md:divide-x divide-slate-100">
              {[
                { title: "เร็วขึ้น ประหยัดเวลา", desc: "พนักงานหน้าไลน์และแผนก QA ค้นข้อมูลและรับคำตอบแบบทันท่วงทีภายใน 2 วินาที แทนการเสียเวลาคุ้ยเอกสารหลายชั่วโมง", icon: "⚡" },
                { title: "แม่นยำขึ้น ลดความผิดพลาด", desc: "การันตีข้อมูลที่ใช้อ้างอิงเป็น Rev ล่าสุด ผ่านการอนุมัติแล้วเท่านั้น ลดการทำงานซ้ำซากและงานเสีย (Defect Ratio)", icon: "🎯" },
                { title: "ปลอดภัยขึ้น ใช้ข้อมูลถูกต้อง", desc: "ช่วยยับยั้งการใช้อุณหภูมิเครื่องจักรหรือความเร็วมั่วซั่วตามใจตนเอง ซึ่งอาจสร้างอันตรายร้ายแรงต่อโรงงาน", icon: "🔒" },
                { title: "ความรู้ไม่หาย พัฒนาต่อเนื่อง", desc: "ระบบรวบรวมเทคนิค Kaizen ของผู้ชำนาญการไว้ในคลังกลาง ไม่ให้สูญสลายเมื่อพนักงานเกษียณหรือลาออก", icon: "📈" },
                { title: "เพิ่มประสิทธิภาพ ลดต้นทุน", desc: "ย่นระยะเวลา Onboarding ฝึกสอนงานช่างใหม่เหลือ 3 วัน แทน 2 สัปดาห์ ประหยัดงบสูญเสียในสายการผลิตได้อย่างยั่งยืน", icon: "💰" }
              ].map((outcome, idx) => (
                <div key={idx} className="pt-4 md:pt-0 md:px-4 space-y-2 text-center">
                  <span className="text-3xl block">{outcome.icon}</span>
                  <h5 className="font-extrabold text-xs text-slate-800 leading-tight">{outcome.title}</h5>
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-xl max-w-2xl w-full max-h-[85vh] overflow-hidden flex flex-col">
            <div className="bg-[#15329c] text-white p-4.5 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <span className="text-[9px] font-mono font-bold uppercase text-slate-100 bg-white/10 px-1.5 py-0.2 rounded leading-none shrink-0 block mb-0.5">{previewCitation.type}</span>
                  <h4 className="font-extrabold text-xs leading-none">{previewCitation.title}</h4>
                </div>
              </div>
              <button
