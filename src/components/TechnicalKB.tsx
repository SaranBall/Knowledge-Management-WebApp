...
export const TechnicalKB: React.FC<TechnicalKBProps> = ({
  ...
  const handleCreateArticleSubmit = (e: React.FormEvent) => {
    ...
    const preparedArticle: KBArticle = {
      ...
      relatedWIs: newArt.relatedWIs
        ? newArt.relatedWIs.split(",").map((s) => s.trim())
        : [],
      tags: newArt.tags
        ? newArt.tags.split(",").map((s) => s.trim())
        : [newArt.type],
  ...
  const handleProposalSubmit = (e: React.FormEvent) => {
    ...
    const preparedArticle: KBArticle = {
      ...
      solution:
        ...
        "รอกลั่นกรองและจัดส่งมอบองค์ความรู้ความเข้าใจจากผู้เชี่ยวชาญ",
      prevention: "รอกำหนดแผนบำรุงรักษาและป้องกันการเสียซ้ำรอย",
      relatedWIs: [],
      tags: proposalData.tags
        ? proposalData.tags.split(",").map((s) => s.trim())
        : [proposalData.type, "Proposal"],
      author: currentUser.name,
      authorTitle: currentUser.position,
      authorDept:
        getDepartmentById(currentUser.departmentId)?.name ||
  ...
  const filteredArticles = articles.filter((art) => {
    ...
    if (currentUser.role === "Viewer" && art.status !== "Approved")
      return false;
    ...
    const matchStr =
      `${art.title} ${art.problem} ${art.cause} ${art.solution} ${art.prevention} ${art.tags.join(" ")} ${art.relatedWIs.join(" ")}`.toLowerCase();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">
            ดึง Tacit Knowledge (ความรู้ในหัวคนเก่ง) จากผู้เชี่ยวชาญ
            บันทึกเป็นสูตรถอดวิธีแก้ไขอุปสรรค ลดปัญหาความชำนาญสูญหาย
          </p>
        </div>
          ...
          <div className="bg-amber-50 rounded-2xl border border-amber-250 p-5 space-y-3">
            <div className="flex items-center gap-2">
              ...
              <h4 className="font-bold text-amber-900 text-xs sm:text-sm">
                ร่างองค์ความรู้ที่รอการอนุมัติความถูกต้องก่อนเผยแพร่ (
                {articles.filter((a) => a.status === "Pending").length} ฉบับ)
              </h4>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {articles
                ...
                .map((art) => (
                  <div
                    key={art.id}
                    className="bg-white p-4 rounded-xl border border-amber-200 flex flex-col justify-between gap-3 text-xs"
                  >
                    <div>
                      <div className="flex justify-between items-start gap-2">
                        ...
                        <span className="bg-slate-100 px-1.5 py-0.2 rounded font-mono text-[9px] font-semibold">
                          {art.type}
                        </span>
                      </div>
                      <p className="text-slate-400 text-[10px] mt-1 font-mono">
                        จัดทำโดย: {art.author} | แผนก: {art.authorDept}
                      </p>
            ...
            {filteredArticles.length === 0 ? (
              <div className="bg-white p-8 rounded-2xl border border-slate-200 text-center space-y-1 text-slate-400 text-xs">
                ...
                <p className="font-bold text-slate-600">
                  ไม่พบหัวข้อเทคนิคในหมวดนี้
                </p>
                <p className="text-[10px]">
                  ตรวจสอบคำค้นหา หรือกรองตามแท็บอื่นๆ
                </p>
              </div>
            ) : (
                  ...
                  >
                    <div className="flex items-center justify-between text-[10px]">
                      ...
                      <span className="text-slate-400 font-mono">
                        {art.createdAt ? art.createdAt.split("T")[0] : ""}
                      </span>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-1">
                      {art.tags.slice(0, 3).map((t, idx) => (
                        <span
                          key={idx}
                          className="bg-slate-100 text-slate-500 text-[8px] font-medium px-1 rounded"
                        >
                          #{t}
                        </span>
                      ))}
                    ...
                    <div className="pt-2 border-t border-slate-100 flex justify-between items-center text-[10px] text-slate-500">
                      <span>
                        โดย{" "}
                        {art.author ? art.author.split(" ")[0] : "เจ้าหน้าที่"}
                      </span>
                      <span className="flex items-center gap-0.5">
                        <Heart className="w-3 h-3 text-rose-500" /> {art.likes}{" "}
                        ถูกใจ
                      </span>
                    </div>
                  </div>
            ...
            <div className="space-y-6 text-xs leading-normal">
              ...
              <div className="border-b border-slate-150 pb-4">
                ...
                <div className="flex items-center gap-2.5 mt-3 pt-2">
                  ...
                  <div>
                    <strong className="block text-slate-800 text-[11px] leading-tight">
                      {selectedArticle.author}
                    </strong>
                    <span className="text-[10px] text-slate-400 font-mono mt-0.5 block">
                      {selectedArticle.authorTitle} | แผนก:{" "}
                      {selectedArticle.authorDept}
                    </span>
                  </div>
                </div>
                ...
                {currentUser.role === "Admin" && (
                  <div className="p-3 bg-blue-50/70 border border-[#e1ded5] rounded-xl flex items-center justify-between gap-2 mt-3 animate-in fade-in">
                    <span className="font-bold text-[10px] text-[#15329c]">
                      เครื่องมือผู้ดูแลระบบ (Admin command):
                    </span>
                    <div className="flex gap-1.5 shrink-0">
                      ...
                      <button
                        ...
                        onClick={() => {
                          if (
                            window.confirm(
                              `คุณแน่ใจว่าต้องการลบบทความวิเคราะห์ปัญหา "${selectedArticle.title}" หรือไม่?`,
                            )
                          ) {
              ...
              <div className="space-y-1 bg-emerald-50/40 p-4 rounded-xl border border-emerald-100">
                ...
                <p className="text-slate-850 leading-relaxed pl-3 font-semibold whitespace-pre-wrap">
              ...
              {selectedArticle.prevention && (
                <div className="space-y-1 bg-blue-50/40 p-4 rounded-xl border border-blue-100">
                  ...
                  <p className="text-slate-755 leading-relaxed pl-3 whitespace-pre-wrap">
                    {selectedArticle.prevention}
                  </p>
                </div>
              )}
              ...
              {selectedArticle.relatedWIs.length > 0 && (
                <div className="p-3.5 bg-neutral-50 rounded-xl border border-slate-200">
                  <span className="text-[10px] font-bold text-slate-400 uppercase block mb-2">
                    เอกสาร WI / QP และแบบฟอร์มมาตรฐานอ้างอิง:
                  </span>
                  <div className="flex flex-wrap gap-2">
                    {selectedArticle.relatedWIs.map((wiCode) => {
                      const matchedItem = documents.find((d) =>
                        d.title.toUpperCase().includes(wiCode.toUpperCase()),
                      );

                      return (
                        <div
                          key={wiCode}
                          className="inline-flex items-center gap-1.5 bg-white border px-2.5 py-1.5 rounded-lg text-xs hover:border-indigo-300"
                        >
              ...
              <div className="pt-4 border-t border-slate-100 flex flex-wrap items-center justify-between gap-2.5 text-[10px] text-slate-400">
                <div className="flex items-center gap-1.5">
                  ...
                  {selectedArticle.tags.map((tag, idx) => (
                    <span
                      key={idx}
                      className="bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded font-mono"
                    >
                      #{tag}
                    </span>
                  ))}
                ...
                <span>
                  สร้างเมื่อ:{" "}
                  {new Date(selectedArticle.createdAt).toLocaleString("th-TH")}
                </span>
              </div>
            </div>
          )}
      ...
      {isFormOpen && (
        <div
          id="add-kb-article-dialog"
          className="fixed inset-0 z-50 bg-slate-900/60 flex items-center justify-center p-4 backdrop-blur-2xs"
        >
              ...
              <div className="p-5 space-y-4 text-xs overflow-y-auto flex-1">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1">
                    <label className="font-semibold text-slate-600 block">
                      ประเภทกลุ่มข้อมูลความรู้:
                    </label>
                    <select
                      id="new-kb-type"
                      value={newArt.type}
                      onChange={(e) =>
                        setNewArt({ ...newArt, type: e.target.value as KBType })
                      }
                  ...
                  <div className="space-y-1">
                    <label className="font-semibold text-slate-600 block">
                      รหัส WI/QP อ้างอิงเชื่อมสัมพันธ์:
                    </label>
                    <input
                      ...
                      onChange={(e) =>
                        setNewArt({ ...newArt, relatedWIs: e.target.value })
                      }
                ...
                <div className="space-y-1">
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setNewArt({ ...newArt, problem: e.target.value })
                    }
                    required
                ...
                <div className="space-y-1">
                  <label className="font-semibold text-slate-600 block">
                    2. วิเคราะห์หาสาเหตุที่ทำให้เกิดปัญหานั้น: (Cause -
                    ไม่บังคับสำหรับ FAQ/Kaizen)
                  </label>
                  <textarea
                    id="new-kb-cause"
                    ...
                    value={newArt.cause}
                    onChange={(e) =>
                      setNewArt({ ...newArt, cause: e.target.value })
                    }
                    className="w-full bg-white border border-slate-205 p-2 rounded-lg text-slate-800 text-xs focus:ring-1 focus:ring-indigo-500 focus:outline-none"
                  />
                </div>
                ...
                <div className="space-y-1">
                  <label className="font-semibold text-slate-600 block">
                    3. ลำดับวิธีกดแก้ไขด่วนเพื่อให้สายผลิตวิ่งต่อได้: (Solution)
                  </label>
                  <textarea
                    id="new-kb-solution"
                    ...
                    onChange={(e) =>
                      setNewArt({ ...newArt, solution: e.target.value })
                    }
                    required
                    className="w-full bg-white border border-slate-205 p-2 rounded-lg text-slate-850 text-xs focus:ring-1 focus:ring-indigo-500 focus:outline-none"
                  />
                ...
                <div className="space-y-1">
                  <label className="font-semibold text-slate-600 block">
                    4. แผนงานบำรุงเชิงรักษาระยะยาวป้องกันการเกิดซ้ำ:
                    (Prevention)
                  </label>
                  <textarea
                    ...
                    onChange={(e) =>
                      setNewArt({ ...newArt, prevention: e.target.value })
                    }
                    className="w-full bg-white border border-slate-205 p-2 rounded-lg text-slate-800 text-xs focus:ring-1 focus:ring-indigo-500 focus:outline-none"
                  />
                </div>
                ...
                <div className="space-y-1">
                  <label className="font-semibold text-slate-600 block">
                    คำค้นหา/แฮชแท็ก: (คั่นด้วยจุลภาค ",")
                  </label>
                  <input
                    id="new-kb-tags"
                    type="text"
                    ...
                    value={newArt.tags}
                    onChange={(e) =>
                      setNewArt({ ...newArt, tags: e.target.value })
                    }
                    className="w-full bg-white border border-slate-205 p-2 rounded-lg text-slate-800 text-xs focus:ring-1 focus:ring-indigo-500 focus:outline-none"
                  />
                </div>
      ...
      {isProposalOpen && (
        ...
        >
          <div className="bg-white rounded-2xl w-full max-w-lg max-h-[90vh] flex flex-col shadow-xl overflow-hidden animate-in fade-in zoom-in-95 duration-150 border border-slate-250 text-left">
            <div className="bg-emerald-950 text-emerald-200 px-5 py-4 flex items-center justify-between text-xs border-b border-emerald-900 shrink-0">
              <div className="flex items-center gap-2">
                ...
                <span className="font-bold text-white">
                  เสนอหัวข้อความรู้ใหม่ / ข้อมูลที่น่าสนใจ (New KM Proposal)
                </span>
              </div>
              <button
            ...
            <form
              id="form-create-km-proposal"
              onSubmit={handleProposalSubmit}
              className="flex-1 flex flex-col overflow-hidden"
            >
              <div className="p-5 space-y-4 text-xs overflow-y-auto flex-1">
                <div className="bg-emerald-50 text-emerald-800 p-3 rounded-xl border border-emerald-150 text-[11px] leading-relaxed">
                  🚀 ร่วมเป็นส่วนหนึ่งของการแบ่งปันความรู้หน้างาน!
                  ทุกข้อเสนอของคุณจะถูกบันทึกไปที่ระบบอบรมในสถานะ{" "}
                  <strong>Pending</strong> เพื่อให้ Admin / ผู้เชี่ยวชาญ
                  คัดกรองและอนุมัติพัฒนาเนื้อหาลงระบบต่อไป
                </div>

                {/* Title field */}
                <div className="space-y-1">
                  <label className="font-semibold text-slate-600 block">
                    หัวข้อความรู้ที่เสนอ หรือ ปัญหาที่ต้องการให้จัดทำ: (KM
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1">
                    ...
                    >
                      ...
                      <option value="Best Practice">
                        Best Practice (แนวทางปฎิบัติงานที่เป็นเลิศ)
                      </option>
                      <option value="Lesson Learned">
                        Lesson Learned (บทเรียนข้อผิดพลาด)
                      </option>
                      <option value="Kaizen">
                        Kaizen (การทำไคเซ็นปรับปรุงงาน)
                      </option>
                ...
                <div className="space-y-1">
                  <label className="font-semibold text-slate-600 block">
                    รายละเอียดเนื้อหา หรือเรื่องราวที่ต้องการเสนอแนะ: (Details){" "}
                ...
                <div className="space-y-1">
                  <label className="font-semibold text-slate-600 block">
                    แนวทางหรือวิธีแก้ไขเบื้องต้น (ถ้ามีสิ่งแนะนำ):
                  </label>
                  <textarea
                ...
                <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-150 text-[10px] text-slate-500 flex items-center justify-between">
                  <span>
                    👤 ผู้เสนอข้อมูล: <strong>{currentUser.name}</strong>
                  </span>
                  <span>
                    สังกัดฝ่าย:{" "}
                    <strong>
                  ...
                  <span>
                    บทบาท: <strong>{currentUser.role}</strong>
                  </span>
                </div>
              </div>
          ...
          <div className="bg-white rounded-2xl w-full max-w-xl max-h-[90vh] flex flex-col shadow-xl overflow-hidden animate-in fade-in zoom-in-95 duration-150 border border-[#e1ded5]">
            <div className="bg-[#15329c] text-white px-5 py-4 flex items-center justify-between text-xs shrink-0">
              <span className="font-bold uppercase text-white">
                แก้ไขข้อมูลเทคโนโลยีบทความ (Admin Edit Mode)
              </span>
              <button
              ...
              <div className="p-5 space-y-4 text-xs overflow-y-auto flex-1">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setEditingArt({ ...editingArt, title: e.target.value })
                      }
                      required
                ...
                <div className="space-y-1">
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setEditingArt({ ...editingArt, problem: e.target.value })
                    }
                    required
                    className="w-full bg-white border border-slate-202 p-2 rounded-lg text-slate-805 text-xs focus:ring-1 focus:ring-[#15329c] focus:outline-none"
                  />
                ...
                <div className="space-y-1">
                  <label className="font-semibold text-slate-600 block">
                    2. ตรวจค้นหาสาเหตุหลัก / ปรากฏการณ์เครื่องจักรหยุด: (Root
                    Cause - Optional)
                  </label>
                  <textarea
                    id="edit-kb-cause"
                    ...
                    onChange={(e) =>
                      setEditingArt({ ...editingArt, cause: e.target.value })
                    }
                    className="w-full bg-white border border-slate-202 p-2 rounded-lg text-slate-805 text-xs focus:ring-1 focus:ring-[#15329c] focus:outline-none"
                  />
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setEditingArt({ ...editingArt, solution: e.target.value })
                    }
                    required
                ...
                <div className="space-y-1">
                  <label className="font-semibold text-slate-600 block">
                    4. มาตรการป้องกันเชิงรักษาระยะยาว: (Prevention - Optional)
                  </label>
                  <textarea
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1">
                    <label className="font-semibold text-slate-600 block text-[10px]">
                      คำค้นหา/คำค้นเพิ่มเติม: (คั่นด้วย ",")
                    </label>
                    <input
                      id="edit-kb-tags"
                      type="text"
                      ...
                      onChange={(e) =>
                        setEditingArt({ ...editingArt, tags: e.target.value })
                      }
                      className="w-full bg-white border border-slate-200 p-2 rounded-lg text-slate-800 text-xs"
                    />
                  </div>
