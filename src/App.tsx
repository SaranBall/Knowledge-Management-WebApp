...
export default function App() {
  ...
  const [kbArticles, setKbArticles] =
    useState<KBArticle[]>(INITIAL_KB_ARTICLES);
  ...
  const [searchLogs, setSearchLogs] =
    useState<SearchLog[]>(INITIAL_SEARCH_LOGS);
  ...
  const [userCompetencies, setUserCompetencies] = useState<UserCompetency[]>(
    () => {
      return INITIAL_USERS.flatMap((u) =>
  ...
  const [kmContributionLogs, setKmContributionLogs] = useState<
    KMContributionLog[]
  >(() => {
  ...
  const [matchedEmployee, setMatchedEmployee] = useState<EmployeeMaster | null>(
    null,
  );
  ...
  const handleAddUser = async (user: UserType) => {
    ...
    addAuditLog(
      "APPROVE_MEMBER",
      `อนุมัติพนักงานใหม่และให้สิทธิ์เข้าใช้ระบบ: ${user.name} (${user.employeeId}) แผนก ${user.departmentId} สิทธิ์ ${user.role}`,
    );
    try {
  ...
  const handleUpdateUser = async (updatedUsr: UserType) => {
    ...
    if (prevUsr) {
      if (prevUsr.name !== updatedUsr.name)
        changes.push(`ชื่อ: ${prevUsr.name} -> ${updatedUsr.name}`);
      if (prevUsr.role !== updatedUsr.role)
        changes.push(`สิทธิ์: ${prevUsr.role} -> ${updatedUsr.role}`);
      if (prevUsr.departmentId !== updatedUsr.departmentId)
        changes.push(
          `แผนก: ${prevUsr.departmentId} -> ${updatedUsr.departmentId}`,
        );
      if (prevUsr.position !== updatedUsr.position)
        changes.push(`ตำแหน่ง: ${prevUsr.position} -> ${updatedUsr.position}`);
      if (prevUsr.status !== updatedUsr.status)
        changes.push(
          `สถานะ: ${prevUsr.status || "Active"} -> ${updatedUsr.status || "Active"}`,
        );
    }
    const changeDesc =
      changes.length > 0
        ? changes.join(", ")
        : "แก้ไขรหัสผ่าน PIN หรือเบอร์ติดต่อ";
    ...
    addAuditLog(
      "UPDATE_MEMBER",
      `แก้ไขประวัติสมาชิก: ${updatedUsr.name} (${updatedUsr.employeeId}) [${changeDesc}]`,
    );
    try {
  ...
  const handleUpdateUserRole = async (id: string, role: Role) => {
    ...
    if (usr) {
      addAuditLog(
        "UPDATE_ROLE",
        `เปลี่ยนสิทธิ์ผู้ใช้: ${usr.name} (${usr.employeeId}) -> ${role}`,
      );
      try {
  ...
  const handleDeleteUser = async (id: string) => {
    ...
    if (usr) {
      addAuditLog(
        "DELETE_MEMBER",
        `เพิกถอนและลบบัญชีออกจากระบบถาวร: ${usr.name} (${usr.employeeId})`,
      );
      try {
        ...
        copy[idx] = {
          ...
          completedDate:
            status === "Completed"
              ...
              : copy[idx].completedDate,
          attemptsCount:
            copy[idx].attemptsCount + (status === "Completed" ? 1 : 0),
        };
  ...
  const handleAddContactReply = async (reqId: string, replyText: string) => {
    setContactRequests((prev) =>
      prev.map((req) =>
        req.id === reqId
          ? { ...req, status: "Replied", replyMessage: replyText }
          : req,
      ),
    );
    try {
  ...
  const handleVerifyEmployeeId = (searchId: string) => {
    ...
    const match = employeeMaster.find(
      (e) => e.employeeId.toLowerCase() === cleanId.toLowerCase(),
    );

    if (!match) {
      setRegError(
        `❌ ไม่พบรหัสพนักงาน "${cleanId}" ในฐานข้อมูลระเบียนระบบ! กรุณาแจ้งแอดมินหรือผู้ควบคุมดูแลให้อัปโหลดไฟล์ Excel/PDF หรือกรอกเข้าระบบสมาชิกก่อน`,
      );
      return;
    }
    ...
    const alreadyRegistered = users.some(
      (u) => u.employeeId.toLowerCase() === cleanId.toLowerCase(),
    );
    if (alreadyRegistered) {
      setRegError(
        `⚠️ รหัสพนักงาน "${cleanId}" นี้เคยได้ลงทะเบียนเปิดใช้งานไปเรียบร้อยแล้ว! สามารถสลับบัญชีเพื่อเข้าเรียนหลักสูตรได้ทันที`,
      );
      return;
    }
  ...
  const handleConfirmRegistration = async () => {
    ...
    if (!cleanPass) {
      setRegError(
        "⚠️ กรุณารองรับความปลอดภัยโดยกำหนดรหัสผ่าน PIN ตัวเลข 6 หลัก",
      );
      return;
    }
  ...
  const menuList = [
    ...
    },
    {
      ...
      icon: Award,
      sub: "ปฐมนิเทศ คอร์สพัฒนา & ใบเซอร์ฯ",
    },
    {
      id: "Technical Knowledge Base",
      label: "Technical Knowledge Base",
      icon: Heart,
      sub: "คลังสมองช่างเทคนิค",
    },
    {
      id: "Expert Directory",
      label: "Expert Directory",
      icon: Users,
      sub: "ด่วน หาผู้เชี่ยวชาญถาม",
    },
    {
      id: "AI Knowledge Assistant",
      label: "AI Knowledge Assistant (RAG)",
      icon: Bot,
      sub: "ระบบสืบค้นคู่มืออัจฉริยะ",
    },
    {
      id: "Member Management",
      label: "Member & Reports Control",
      icon: Shield,
      sub: "จัดการสิทธิ์และออกรายงาน",
    },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#f0f4fa] font-sans flex flex-col text-[#1e293b] selection:bg-[#e51a24] selection:text-white">
      ...
      {!isLogged ? (
        <div
          id="login-portal"
          className="flex-1 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 bg-gradient-to-tr from-[#f5f4ed] via-[#fcfbf9] to-[#ffffff] text-slate-800"
        >
            ...
            <div className="space-y-4">
              ...
              <div className="pt-2">
                ...
                <p className="text-slate-500 text-xs max-w-xs mx-auto mt-3 leading-relaxed">
                  ระบบบริหารสารสนเทศและพัฒนาความรู้เพื่อเตรียมความพร้อมสู่การตรวจประเมิน
                  ISO 9001 / HACCP และลดความสูญเสียทักษะองค์กร (Loss Prevention)
                </p>
              </div>
            </div>
            ...
            {!isRegistering ? (
              ...
              <div className="space-y-5 pt-4 border-t border-[#e1ded5] text-left">
                ...
                <form onSubmit={handleManualLogin} className="space-y-3.5">
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-slate-500 block uppercase">
                      รหัสผ่านความปลอดภัย PIN (6 Digits):
                    </label>
                    <div className="relative">
                      <span className="absolute inset-y-0 left-0 flex items-center pl-3 text-slate-400">
                      ...
                      <input
                        ...
                        value={loginPassword}
                        onChange={(e) =>
                          setLoginPassword(e.target.value.replace(/\D/g, ""))
                        }
                ...
                {!matchedEmployee ? (
                  ...
                  <div className="space-y-3.5">
                    <p className="text-[11px] text-slate-500 leading-relaxed">
                      กรอกเพียง <strong>รหัสพนักงาน</strong>{" "}
                      เพื่อระบุความถูกต้อง ระบบจะสืบค้นแผนก ฝ่าย ชื่อนามสกุล
                      และระดับจัดเก็บของคุณจากฐานข้อมูลนำเข้าความปลอดภัยกลาง
                    </p>

                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-slate-500 block uppercase">
                        รหัสพนักงานอ้างอิงของคุณ:
                      </label>
                    ...
                    <div className="p-3 bg-amber-50 rounded-xl border border-amber-200/60 shadow-xs">
                      <span className="block text-[9.5px] font-bold text-amber-800 uppercase tracking-wider mb-2">
                        💡 รหัสพนักงานรอลงทะเบียน (สุ่มจำลองจากการนำเข้า):
                      </span>
                      <div className="flex flex-col gap-1.5 text-[10.5px] font-mono text-slate-700">
                        {employeeMaster.filter((e) => e.status === "Imported")
                          .length === 0 ? (
                          <span className="text-slate-400 italic">
                            ไม่มีข้อมูลพนักงานรอนำเข้า (ลงทะเบียนครบหมดแล้ว)
                            สามารถกด Import เพิ่มใหม่จากหน้า Member Management
                          </span>
                        ) : (
                          employeeMaster
                  ...
                  <div className="space-y-4">
                    <div className="p-4 bg-gradient-to-br from-[#15329c]/5 to-indigo-50 border border-indigo-150 rounded-2xl space-y-3">
                      <div className="flex items-center gap-2 border-b border-indigo-100/60 pb-2">
                        ...
                        <span className="text-xs font-black text-slate-800 uppercase tracking-widest">
                          พบบันทึกการอนุมัตินำเข้าพนักงาน
                        </span>
                      </div>
                      ...
                      <div className="space-y-1.5 text-xs text-slate-700">
                        ...
                        <div className="grid grid-cols-3">
                          ...
                          <span className="col-span-2 font-bold text-[#15329c]">
                        ...
                        <div className="grid grid-cols-3">
                          ...
                          <span className="col-span-2 text-slate-800 font-semibold">
                            {getDepartmentById(matchedEmployee.departmentId)
                              ?.name || matchedEmployee.departmentId}
                          </span>
                        </div>
                        <div className="grid grid-cols-3">
                          <span className="text-slate-400 font-medium">
                        ...
                        <div className="grid grid-cols-3">
                          ...
                          <span className="col-span-2 text-slate-600 font-mono text-[11px] truncate">
                            {matchedEmployee.email} • {matchedEmployee.phone}
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-2">
                      ...
                      <input
                        ...
                        value={regPassword}
                        onChange={(e) =>
                          setRegPassword(e.target.value.replace(/\D/g, ""))
                        }
        ...
        <div className="flex-1 flex flex-col md:flex-row relative">
          ...
          <aside
            id="sidebar-menu"
            className="w-full md:w-64 bg-white text-slate-700 border-r border-[#e1ded5] flex flex-col justify-between shrink-0 h-auto md:h-screen sticky top-0 z-20"
          >
            <div>
              ...
              <nav
                className={`p-2.5 space-y-1 ${mobileMenuOpen ? "block" : "hidden md:block"}`}
              >
                {menuList.map((menu) => {
                  ...
                  return (
                    ...
                    >
                      <Icon
                        className={`w-4 h-4 shrink-0 transition-transform group-hover:scale-110 ${isActive ? "text-white" : "text-slate-400"}`}
                      />
                      <div className="min-w-0">
                        ...
                        {menu.sub && (
                          <span
                            className={`text-[8px] font-mono mt-0.5 block truncate ${isActive ? "text-blue-100 opacity-90" : "text-slate-400"}`}
                          >
                            {menu.sub}
                          </span>
                        )}
            ...
            <div
              className={`p-4 border-t border-[#e1ded5] bg-[#fbfbf9] space-y-3 ${mobileMenuOpen ? "block" : "hidden md:block"}`}
            >
              ...
              <div className="text-[8.5px] text-slate-550 leading-normal border-t border-[#e1ded5]/80 pt-2 flex items-center justify-between">
                ...
                <span className="font-mono text-[#e51a24]/80 text-[7px] tracking-widest font-bold">
                  日本製 5S
                </span>
              </div>
            </div>
